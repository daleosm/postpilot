...
test.describe("Post work orders", () => {
  ...
  test.beforeAll(async () => {
    ...
    await sql`insert into users (id, name, email) values (${qcUserId}, 'QC Lab Operator', 'qc-lab@postpilot.test'), (${editorUserId}, 'Editorial Lab Operator', 'editor-lab@postpilot.test') on conflict (id) do update set name = excluded.name`;
    await sql`insert into organizations (id, name, slug) values (${organizationId}, 'Work Order Lab', 'work-order-lab')`;
    await sql`insert into organization_members (organization_id, user_id, role) values (${organizationId}, 'user_maya', 'admin'), (${organizationId}, ${qcUserId}, 'member'), (${organizationId}, ${editorUserId}, 'member')`;
  ...
  test("requires finance confirmation before a completed client change reaches the episode budget", async ({ page }) => {
    ...
    expect(switchUser.status()).toBe(200);
    await activateLab(page);
    const create = await page.request.post("/api/work-orders", { data: { episodeId, workflowStageId: stageId, title: "Client-requested alternate title card", billingScope: "billable_change", estimatedAmount: 750, currency: "USD" } });
    expect(create.status()).toBe(201);
    const workOrderId = (await create.json()).id as string;
