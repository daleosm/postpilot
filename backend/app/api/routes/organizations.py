...
from app.api.schemas import ActiveOrganizationRequest, ActiveShowRequest, SessionResponse
from app.auth import get_actor_for_token, set_active_organization, set_active_show
...
@router.post("/active-show", response_model=SessionResponse)
async def switch_active_show(
    payload: ActiveShowRequest, request: Request, session: DbSession, actor: CurrentActor
) -> SessionResponse:
    if payload.show_id:
        # Limited users cannot use the persistent switcher to probe or retain
        # a show outside their own assigned episode scope.
        await require_show_access(session, actor, payload.show_id)
    await set_active_show(session, actor, payload.show_id)
    token = request.cookies.get(get_settings().cookie_name)
    if not token:
        raise RuntimeError("Authenticated request lost its session cookie.")
