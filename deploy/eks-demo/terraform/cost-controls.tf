# Free early-warning controls for a public demo. These alerts do not stop
# resources or alter IAM; compute/node limits remain the hard cost boundary.
# Billing data is delayed, so treat this as notification rather than a kill
# switch. Keeping it opt-in avoids creating subscriptions without an owner.
locals {
