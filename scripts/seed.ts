...
async function seedTenant(tenant: TenantSeed) {
  ...
  await db.insert(bookings).values([
    { id: bookingId(1), organizationId: tenant.id, roomId: roomId(1), episodeId: episodeId(1), personId: byRole("editor"), title: `${episodeRows[0].productionCode} editorial block`, startsAt: at(0, 9), endsAt: at(1, 18), setupMinutes: 15, handoverMinutes: 15, status: "confirmed", bookingType: "edit", notes: "Two-day editorial booking; client links remain external." },
    { id: bookingId(2), organizationId: tenant.id, roomId: roomId(3), episodeId: episodeId(4), personId: byRole("colorist"), title: `${episodeRows[3].productionCode} grade pass`, startsAt: at(1, 9), endsAt: at(2, 18), setupMinutes: 45, handoverMinutes: 15, status: "confirmed", bookingType: "color", notes: "Finishing review references are maintained in notes." },
    { id: bookingId(3), organizationId: tenant.id, roomId: roomId(4), episodeId: episodeId(7), personId: byRole("sound_mixer"), title: `${episodeRows[6].productionCode} final mix`, startsAt: at(2, 10), endsAt: at(3, 19), setupMinutes: 30, handoverMinutes: 30, status: "confirmed", bookingType: "mix" },
    { id: bookingId(4), organizationId: tenant.id, roomId: roomId(5), episodeId: episodeId(4), personId: byRole("qc"), title: `${episodeRows[3].productionCode} technical QC`, startsAt: at(3, 9), endsAt: at(3, 16), setupMinutes: 30, handoverMinutes: 0, status: "confirmed", bookingType: "qc" },
    ...
    ...(episodeRows[8] ? [{ id: bookingId(9), organizationId: tenant.id, roomId: roomId(2), episodeId: episodeId(9), personId: byRole("editor"), title: `${episodeRows[8].productionCode} client pencil hold`, startsAt: at(4, 10), endsAt: at(4, 15), isOption: true, optionRank: 1, status: "tentative" as const, bookingType: "edit" as const, notes: "First pencil hold; confirm only once the client releases the room." }] : []),
    ...(episodeRows[9] ? [{ id: bookingId(10), organizationId: tenant.id, roomId: roomId(2), episodeId: episodeId(10), personId: byRole("editor"), title: `${episodeRows[9].productionCode} alternate client pencil`, startsAt: at(4, 10), endsAt: at(4, 15), isOption: true, optionRank: 2, status: "tentative" as const, bookingType: "edit" as const, notes: "Second pencil hold competing for the same editorial window." }] : []),
  ]);
  ...
  await db.insert(postWorkOrders).values([
    ...
    ...(tenant.number === 3 ? [{ id: id(tenant.number, "38", 3), organizationId: tenant.id, episodeId: episodeId(1), workflowStageId: stageId(2), workType: "internal" as const, title: "Prep revised captions and turnover package", description: "Prepare the revised caption reference, confirm the turnover checklist, and hand off the package for the next finishing pass.", department: "Editorial", assigneePersonId: byRole("assistant_editor"), assigneeRole: "assistant_editor", priority: "normal" as const, isBlocking: false, status: "in_progress" as const, billingScope: "internal" as const, currency: tenant.budgetProfile.currency, dueAt: at(1, 16) }] : []),
