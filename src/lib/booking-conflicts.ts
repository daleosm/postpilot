...
type BookingWindow = { roomId?: string | null; personId?: string | null; startsAt: Date; endsAt: Date; setupMinutes?: number; handoverMinutes?: number; bookingType?: string; excludeId?: string; includeOptionBookings?: boolean };
...
export async function findBookingConflicts(organizationId: string, window: BookingWindow) {
  ...
  const conditions = [eq(bookings.organizationId, organizationId), ne(bookings.status, "cancelled"), window.includeOptionBookings === false ? eq(bookings.isOption, false) : undefined, or(...resources), sql`${bookings.startsAt} - (${bookings.setupMinutes} * interval '1 minute') < ${operational.endsAt.toISOString()}::timestamptz`, sql`${bookings.endsAt} + (${bookings.handoverMinutes} * interval '1 minute') > ${operational.startsAt.toISOString()}::timestamptz`];
...
export async function getBookingSuggestions(organizationId: string, window: BookingWindow) {
  ...
  const [availableRooms, availablePeople, resourceBookings] = await Promise.all([
    ...
    (window.roomId || window.personId) ? db.select({ startsAt: bookings.startsAt, endsAt: bookings.endsAt, setupMinutes: bookings.setupMinutes, handoverMinutes: bookings.handoverMinutes }).from(bookings).where(and(eq(bookings.organizationId, organizationId), ne(bookings.status, "cancelled"), window.includeOptionBookings === false ? eq(bookings.isOption, false) : undefined, or(window.roomId ? eq(bookings.roomId, window.roomId) : undefined, window.personId ? eq(bookings.personId, window.personId) : undefined), sql`${bookings.endsAt} + (${bookings.handoverMinutes} * interval '1 minute') > ${operational.startsAt.toISOString()}::timestamptz`, window.excludeId ? ne(bookings.id, window.excludeId) : undefined)).orderBy(asc(bookings.startsAt)) : [],
  ]);
  ...
  const busyAlternatives = alternativeResourceConditions.length ? await db.select({ roomId: bookings.roomId, personId: bookings.personId }).from(bookings)
    .where(and(eq(bookings.organizationId, organizationId), ne(bookings.status, "cancelled"), window.includeOptionBookings === false ? eq(bookings.isOption, false) : undefined, or(...alternativeResourceConditions), sql`${bookings.startsAt} - (${bookings.setupMinutes} * interval '1 minute') < ${operational.endsAt.toISOString()}::timestamptz`, sql`${bookings.endsAt} + (${bookings.handoverMinutes} * interval '1 minute') > ${operational.startsAt.toISOString()}::timestamptz`, window.excludeId ? ne(bookings.id, window.excludeId) : undefined)) : [];
