...
export const crmCompanies = pgTable("crm_companies", { id: uuid("id").defaultRandom().primaryKey(), organizationId: uuid("organization_id").notNull().references(() => organizations.id, { onDelete: "cascade" }), name: text("name").notNull(), type: crmCompanyType("type").notNull(), address: text("address"), serviceCategory: text("service_category"), isPreferredSupplier: boolean("is_preferred_supplier").default(false).notNull(), paymentTermsDays: integer("payment_terms_days"), currency: text("currency").default("GBP").notNull(), financeEmail: text("finance_email"), billingEmail: text("billing_email"), accountStatus: crmAccountStatus("account_status").default("active").notNull(), notes: text("notes"), ...auditColumns }, (table) => [uniqueIndex("crm_companies_org_name_idx").on(table.organizationId, table.name), index("crm_companies_org_type_idx").on(table.organizationId, table.type), index("crm_companies_org_status_idx").on(table.organizationId, table.accountStatus), index("crm_companies_org_service_idx").on(table.organizationId, table.serviceCategory)]);
