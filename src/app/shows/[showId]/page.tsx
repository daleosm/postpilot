...
export default async function ShowDetailPage({ params }: { params: Promise<{ showId: string }> }) {
  ...
  return <div className="space-y-5"><div className="flex items-center justify-between gap-3"><Link href="/shows" className="flex items-center gap-1 text-xs font-medium text-[#617b75]"><ArrowLeft size={14} /> All shows</Link>{mayManageShows && <div className="flex items-center gap-2"><EpisodeFormDialog seasons={seasons.map((season) => ({ id: season.id, label: `${show.title} · Season ${season.number}` }))} people={people} defaultSeasonId={seasons[0]?.id} /><ShowFormDialog show={show} companies={companies} /></div>}</div>
    ...
    <div id="episode-board"><Panel title="Episode board" icon={<Clapperboard size={16} />}><div className="divide-y divide-[#efeeea]">{episodes.map((episode) => <Link key={episode.id} href={`/episodes/${episode.id}`} className="flex items-center justify-between gap-3 px-5 py-3 transition hover:bg-[#fbfbf9]"><div className="min-w-0"><p className="truncate text-sm font-medium text-[#3b423f]">E{String(episode.number).padStart(2, "0")} · {episode.title}</p><p className="mt-0.5 truncate text-xs text-[#858a87]">{episode.workflowStage ?? "Workflow not configured"} · {episode.editorName ?? "Unassigned"}</p></div><WorkflowStateBadge status={episode.status} className="shrink-0" /></Link>)}</div></Panel></div>
