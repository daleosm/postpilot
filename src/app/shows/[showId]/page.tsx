...
import { ArrowLeft, CalendarRange, Clapperboard, ContactRound, UsersRound } from "lucide-react";
...
export default async function ShowDetailPage({ params }: { params: Promise<{ showId: string }> }) {
  ...
  return <div className="space-y-5"><div className="flex items-center justify-between gap-3"><Link href="/shows" className="flex items-center gap-1 text-xs font-medium text-[#617b75]"><ArrowLeft size={14} /> All shows</Link><div className="flex items-center gap-2"><EpisodeFormDialog seasons={seasons.map((season) => ({ id: season.id, label: `${show.title} · Season ${season.number}` }))} people={people} defaultSeasonId={seasons[0]?.id} /><ShowFormDialog show={show} /></div></div>
    ...
    <section className="grid gap-4 xl:grid-cols-[1.35fr_0.9fr]"><Panel title="Seasons & episodes" icon={<CalendarRange size={16} />}><div className="divide-y divide-[#efeeea]">{seasons.map((season) => <div key={season.id} className="flex items-center justify-between px-5 py-3"><div><p className="text-sm font-medium text-[#3b423f]">Season {season.number}</p><p className="mt-0.5 text-xs text-[#858a87]">{season.activeCount} active of {season.episodeCount} episodes</p></div><Link href={`/episodes?season=${season.id}`} className="text-xs font-semibold text-[#58766e] hover:text-[#365f53] hover:underline">Open season <span aria-hidden>→</span></Link></div>)}</div></Panel><Panel title="Episode team" icon={<UsersRound size={16} />}><div className="divide-y divide-[#efeeea]">{team.length ? team.map((person) => <div key={person.id} className="px-4 py-3"><p className="truncate text-xs font-semibold text-[#464d49]">{person.name} <span className="font-normal capitalize text-[#858a87]">· {person.role.replaceAll("_", " ")}</span></p><div className="mt-2 flex flex-wrap gap-1">{person.episodes.map((episode) => <Link key={episode.id} href={`/episodes/${episode.id}`} className="rounded bg-[#edf2ee] px-1.5 py-1 text-[10px] font-medium text-[#4f6e64] hover:bg-[#e2ebe5]">S{episode.seasonNumber} · E{String(episode.number).padStart(2, "0")} {episode.title}</Link>)}</div></div>) : <p className="px-4 py-8 text-center text-sm text-[#858a87]">Assign people from each episode.</p>}</div></Panel></section>
...
async function getShowDetail(showId: string) {
  if (isDebugDemoMode) {
    ...
    return { show: { ...show, productionCompany: show.title === "Signal North" ? "Vantage Television" : "Independent production" }, seasons: show.seasons.map((season) => ({ id: season.id, number: season.number, episodeCount: season.episodeCount, activeCount: season.activeEpisodeCount })), episodes, team, people, contacts: [], contactOptions: [], activity: demo.dashboard.activity };
  }
