...
export default async function ShowDetailPage({ params }: { params: Promise<{ showId: string }> }) {
  ...
  if (!data) notFound();
  const { show, seasons, episodes, team, availableTeam, activity } = data;
  return <div className="space-y-5"><div className="flex items-center justify-between gap-3"><Link href="/shows" className="flex items-center gap-1 text-xs font-medium text-[#617b75]"><ArrowLeft size={14} /> All shows</Link><div className="flex items-center gap-2"><EpisodeFormDialog seasons={seasons.map((season) => ({ id: season.id, label: `${show.title} · Season ${season.number}` }))} people={availableTeam} defaultSeasonId={seasons[0]?.id} /><ShowFormDialog show={show} people={availableTeam} assignedTeamIds={team.map((person) => person.id)} /></div></div>
...
async function getShowDetail(showId: string) {
  if (isDebugDemoMode) {
    ...
    return { show: { ...show, productionCompany: show.title === "Signal North" ? "Vantage Television" : "Independent production" }, seasons: show.seasons.map((season) => ({ id: season.id, number: season.number, episodeCount: season.episodeCount, activeCount: season.activeEpisodeCount })), episodes, team, availableTeam: team, activity: demo.dashboard.activity };
  }
