...
import { Activity, ArrowLeft, CalendarRange, Clapperboard, ContactRound, Truck } from "lucide-react";
...
import { SeasonEpisodesDialog } from "@/components/season-episodes-dialog";
...
export default async function ShowDetailPage({ params }: { params: Promise<{ showId: string }> }) {
  ...
  if (!data) notFound();
  const { show, seasons, episodes, people, contacts, activity, companies = [] } = data;
  const recentEpisodes = [...episodes].sort((left, right) => right.seasonNumber - left.seasonNumber || right.number - left.number).slice(0, 5);
  ...
  return <div className="space-y-5"><div className="flex items-center justify-between gap-3"><Link href="/shows" className="flex items-center gap-1 text-xs font-medium text-[#617b75]"><ArrowLeft size={14} /> All shows</Link>{mayManageShows && <div className="flex items-center gap-2"><EpisodeFormDialog seasons={seasons.map((season) => ({ id: season.id, label: `${show.title} · Season ${season.number}` }))} people={people} defaultSeasonId={seasons[0]?.id} /><ShowFormDialog show={show} companies={companies} /></div>}</div>
    ...
    <section className="grid gap-5 xl:grid-cols-2"><Panel title="Seasons & episodes" icon={<CalendarRange size={16} />}><div className="divide-y divide-[#efeeea]">{seasons.map((season) => <div key={season.id} className="flex items-center justify-between px-5 py-3"><div><p className="text-sm font-medium text-[#3b423f]">Season {season.number}</p><p className="mt-0.5 text-xs text-[#858a87]">{season.activeCount} active of {season.episodeCount} episodes</p></div><SeasonEpisodesDialog seasonNumber={season.number} episodes={episodes.filter((episode) => episode.seasonId === season.id)} /></div>)}</div></Panel><Panel title="Recent workflow completions" icon={<Activity size={16} />}><div className="divide-y divide-[#efeeea]">{activity.slice(0, 4).map((item) => <Link href={`/episodes/${item.episodeId}`} key={item.id} className="flex items-start gap-3 px-5 py-3.5 transition hover:bg-[#fbfbf9]"><span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-[#e6eee8] text-[#527968]"><Activity size={13} /></span><div className="min-w-0"><p className="text-sm font-medium text-[#3d4542]">{workflowStageLabel(item.metadata)}</p><p className="mt-0.5 truncate text-xs text-[#6f7773]">S{item.seasonNumber} · E{String(item.episodeNumber).padStart(2, "0")} {item.episodeTitle}</p><p className="mt-1 text-[11px] text-[#969b98]">Completed {formatActivityDate(item.createdAt)}</p></div></Link>)}{!activity.length && <p className="px-5 py-8 text-center text-sm text-[#858a87]">No workflow stages have been completed for this show yet.</p>}</div></Panel></section>
    <div id="episode-board"><Panel title="Episode board" icon={<Clapperboard size={16} />}><div className="divide-y divide-[#efeeea]">{recentEpisodes.map((episode) => <Link key={episode.id} href={`/episodes/${episode.id}`} className="flex items-center justify-between gap-3 px-5 py-3 transition hover:bg-[#fbfbf9]"><div className="min-w-0"><p className="truncate text-sm font-medium text-[#3b423f]">S{episode.seasonNumber} · E{String(episode.number).padStart(2, "0")} · {episode.title}</p><p className="mt-0.5 truncate text-xs text-[#858a87]">{episode.workflowStage ?? "Workflow not configured"} · {episode.editorName ?? "Unassigned"}</p></div><WorkflowStateBadge status={episode.status} className="shrink-0" /></Link>)}{!recentEpisodes.length && <p className="px-5 py-8 text-center text-sm text-[#858a87]">No episodes have been added to this show.</p>}</div></Panel></div>
...
async function getShowDetail(showId: string) {
  try {
    const data = await postpilotApiServerFetch<{
      ...
      episodes: Array<{ id: string; season_id: string; season_number: number; title: string; number: number; workflow_stage: string | null; workflow_status: string; editor_name: string | null }>;
