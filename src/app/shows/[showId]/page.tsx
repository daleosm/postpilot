...
import { Activity, ArrowLeft, CalendarRange, Clapperboard, ContactRound, UsersRound } from "lucide-react";
...
export default async function ShowDetailPage({ params }: { params: Promise<{ showId: string }> }) {
  const [mayManageShows, mayManageBudget, organizationContext] = await Promise.all([can("manage_shows"), can("manage_budget"), getActiveOrganizationContext()]);
