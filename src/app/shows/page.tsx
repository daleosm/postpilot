...
import { getBudgetData, getDemoCommandCenterData, listEpisodes, listShows } from "@/server/data";
...
async function getShowsData() {
  if (isDebugDemoMode) {
    ...
    return { organizationName: demo.organizationName, shows: demo.showRows.map((show, index) => ({ ...show, seasonCount: show.seasons.length, episodeCount: show.seasons.reduce((sum, season) => sum + season.episodeCount, 0), activeEpisodeCount: show.seasons.reduce((sum, season) => sum + season.activeEpisodeCount, 0), budgetHealth: [90, 84, 96][index] })) };
  }
  const context = await getActiveOrganizationContext();
  if (!context?.organization) return null;
  const organizationId = context.organization.organizationId;
  const [shows, episodeRows, budget] = await Promise.all([listShows(organizationId), listEpisodes(organizationId), getBudgetData(organizationId)]);
