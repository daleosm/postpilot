...
async function loadFastApprovalInbox() {
  ...
  return [
    ...
    inbox.work_orders.map((item) => ({ id: item.id, episodeId: item.episode_id, showId: item.show_id, bookingId: item.booking_id, workType: item.work_type, showTitle: item.show_title, episodeTitle: item.episode_title, episodeNumber: item.episode_number, workflowStageName: item.workflow_stage_name, kind: item.kind, title: item.title, description: item.description, priority: item.priority, isBlocking: item.is_blocking, status: item.status, dueAt: item.due_at ? new Date(item.due_at) : null, externalUrl: item.external_url, workflowState: item.workflow_state ? { displayStatus: item.workflow_state.display_status, primaryStageName: item.workflow_state.primary_stage_name } : null })),
    inbox.has_workspace,
  ] as const;
}
