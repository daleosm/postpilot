...
async function loadFastApprovalInbox() {
  const inbox = await postpilotApiServerFetch<{
    ...
    work_orders: Array<{ id: string; episode_id: string; show_id: string; booking_id: string | null; work_type: string; show_title: string; episode_title: string; episode_number: number; workflow_stage_name: string | null; kind: string; title: string; description: string | null; is_blocking: boolean; status: string; due_at: string | null; external_url: string | null; workflow_state: { display_status: string; primary_stage_name: string | null } | null }>;
  }>("/approvals");
  return [
    ...
    inbox.work_orders.map((item) => ({ id: item.id, episodeId: item.episode_id, showId: item.show_id, bookingId: item.booking_id, workType: item.work_type, showTitle: item.show_title, episodeTitle: item.episode_title, episodeNumber: item.episode_number, workflowStageName: item.workflow_stage_name, kind: item.kind, title: item.title, description: item.description, isBlocking: item.is_blocking, status: item.status, dueAt: item.due_at ? new Date(item.due_at) : null, externalUrl: item.external_url, workflowState: item.workflow_state ? { displayStatus: item.workflow_state.display_status, primaryStageName: item.workflow_state.primary_stage_name } : null })),
    inbox.has_workspace,
  ] as const;
}
