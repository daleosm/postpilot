...
async function load() {
  ...
  const [resources, requests] = await Promise.all([
    postpilotApiServerFetch<{ rooms: Array<{ id: string; name: string; type: string }>; active_booking: { id: string; room_id: string; room_name: string; coverage: "scheduled" | "actual_time" | "overtime_pending" } | null; active_work_order: { id: string; episode_id: string; title: string } | null }>("/catering/resources"),
  ...
  return {
    ...
    resources: { rooms: resources.rooms, activeBooking: resources.active_booking ? { id: resources.active_booking.id, roomId: resources.active_booking.room_id, roomName: resources.active_booking.room_name, coverage: resources.active_booking.coverage } : null, activeWorkOrder: resources.active_work_order ? { id: resources.active_work_order.id, episodeId: resources.active_work_order.episode_id, title: resources.active_work_order.title } : null },
