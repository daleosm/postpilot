...
export async function PATCH(request: Request, { params }: { params: Promise<{ episodeId: string }> }) {
  ...
  if (episode.workflowStageId && episode.workflowStageId !== stage.id) {
    const [currentStage] = await db.select({ id: workflowStages.id, position: workflowStages.position, requiresQcPass: workflowStages.requiresQcPass, deliveryGate: workflowStages.deliveryGate }).from(workflowStages).where(and(eq(workflowStages.id, episode.workflowStageId), eq(workflowStages.organizationId, organizationId))).limit(1);
    ...
    if (isNextStage) {
      ...
      if (currentStage.deliveryGate !== "none") {
        const readiness = await getDeliveryWorkflowGateReadiness({ organizationId, episodeId: episode.id, workflowStageId: currentStage.id, deliveryGate: currentStage.deliveryGate });
        if (!readiness.ready) return NextResponse.json({ error: readiness.message ?? "Required delivery items are not ready for sign-off." }, { status: 409 });
      }
      ...
      if (rules.some((rule) => rule.isRequired && !approvals.some((approval) => approval.approvalRuleId === rule.id && approval.status === "approved"))) return NextResponse.json({ error: "Complete every required sign-off for the current stage before advancing." }, { status: 409 });
    } else {
      if (!stage.canStartEarly || stage.position <= currentStage.position) return NextResponse.json({ error: "Workflow stages normally proceed in order. Enable Allow early start on a later stage to run it in parallel." }, { status: 409 });
      startedEarly = true;
    }
  }
  if (startedEarly) {
    await db.insert(episodeWorkflowTracks).values({ organizationId, episodeId: episode.id, workflowStageId: stage.id, status: "in_progress", startedAt: new Date() }).onConflictDoNothing();
  } else {
  ...
  return NextResponse.json({ ok: true, workflowStage: stage, startedEarly });
}
...
export async function POST(request: Request, { params }: { params: Promise<{ episodeId: string }> }) {
  ...
  const [[episode], [stage], [person], [track]] = await Promise.all([
    ...
    db.select({ id: workflowStages.id, workflowId: workflowStages.workflowId, name: workflowStages.name, key: workflowStages.key, position: workflowStages.position, requiresQcPass: workflowStages.requiresQcPass, deliveryGate: workflowStages.deliveryGate }).from(workflowStages).innerJoin(postWorkflows, eq(workflowStages.workflowId, postWorkflows.id)).where(and(eq(workflowStages.id, parsed.data.workflowStageId), eq(workflowStages.organizationId, context.organization.organizationId), eq(postWorkflows.organizationId, context.organization.organizationId))).limit(1),
  ...
  if (stage.deliveryGate !== "none") {
    const readiness = await getDeliveryWorkflowGateReadiness({ organizationId, episodeId, workflowStageId: stage.id, deliveryGate: stage.deliveryGate });
    if (!readiness.ready) return NextResponse.json({ error: readiness.message ?? "Required delivery items are not ready for sign-off." }, { status: 409 });
  }
  ...
  if (existingApproval) {
    const updated = await db.update(episodeWorkflowApprovals).set({ status: "approved", requiredPersonId: person.id, approverPersonId: person.id, comment: parsed.data.comment || null, respondedAt: new Date(), updatedAt: new Date() }).where(and(eq(episodeWorkflowApprovals.id, existingApproval.id), eq(episodeWorkflowApprovals.organizationId, organizationId), eq(episodeWorkflowApprovals.status, "pending"))).returning({ id: episodeWorkflowApprovals.id });
    if (!updated.length) return NextResponse.json({ error: "This sign-off has already been recorded." }, { status: 409 });
  } else {
    const inserted = await db.insert(episodeWorkflowApprovals).values({ organizationId, episodeId, workflowStageId: stage.id, approvalRuleId: candidate.id, approverRole: candidate.approverRole, requiredPersonId, approverPersonId: person.id, status: "approved", comment: parsed.data.comment || null, respondedAt: new Date() }).onConflictDoNothing().returning({ id: episodeWorkflowApprovals.id });
    if (!inserted.length) return NextResponse.json({ error: "This sign-off has already been recorded." }, { status: 409 });
  }
