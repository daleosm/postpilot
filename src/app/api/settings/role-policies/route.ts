...
export async function PATCH(request: Request) {
  ...
  const policies = [...parsed.data.policies.filter((policy) => !isFixedRole(policy.role)), guestRolePolicy];
  const db = getDb();
  ...
  const configuredRoles = new Set(policies.map((policy) => policy.role));
