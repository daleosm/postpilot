...
export async function PATCH(request: Request) {
  ...
  const configuredRoles = new Set(parsed.data.policies.map((policy) => policy.role));
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(organizationRolePolicies).values(parsed.data.policies.map((policy) => ({ organizationId, role: policy.role, label: policy.label, permissions: policy.permissions })));
  });
  await writeAuditEvent({ organizationId, actorUserId: context.userId, action: "organization.role_policies_updated", entityType: "organization", entityId: organizationId, metadata: { roleCount: parsed.data.policies.length } });
