...
export async function POST(_: Request, { params }: { params: Promise<{ submissionId: string }> }) {
  ...
  if (projection && booking.episodeId && !budgetLine) {
    ...
    const [created] = await db.insert(budgetLines).values({ organizationId, showId: episode.showId, seasonId: episode.seasonId, episodeId: booking.episodeId, code: `BOOKING-${projection.category.toUpperCase().replaceAll(/[^A-Z0-9]+/g, "-")}`, category: projection.category, description: "Automatically calculated from approved room and artist time.", budgetedAmount: String(projection.budgetedAmount), actualAmount: String(projection.actualAmount), currency, costType: "internal" }).returning({ id: budgetLines.id, actualAmount: budgetLines.actualAmount });
    budgetLine = created;
  }
  ...
  await db.transaction(async (tx) => {
    ...
    if (projection && budgetLine) await tx.update(budgetLines).set({ budgetedAmount: String(projection.budgetedAmount), actualAmount: String(projection.actualAmount), currency, updatedAt: new Date() }).where(and(eq(budgetLines.id, budgetLine.id), eq(budgetLines.organizationId, organizationId)));
  });
