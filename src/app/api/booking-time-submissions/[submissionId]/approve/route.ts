...
export async function POST(_: Request, { params }: { params: Promise<{ submissionId: string }> }) {
  ...
  const [mayApproveBudgetOverrun, mayApprovePoOverrun] = await Promise.all([can("approve_budget_overruns"), can("approve_po_overruns")]);
  ...
  let budgetLine: { id: string; purchaseOrderId: string | null; actualAmount: string | number } | null = null;
  if (projection && booking.episodeId) {
    const [existing] = await db.select({ id: budgetLines.id, purchaseOrderId: budgetLines.purchaseOrderId, actualAmount: budgetLines.actualAmount }).from(budgetLines)
    ...
    if (budgetLine?.purchaseOrderId) {
      try {
        await checkPurchaseOrderAllocation(organizationId, budgetLine.purchaseOrderId, projection.actualAmount - Number(budgetLine.actualAmount), "vendor_commitment", mayApprovePoOverrun);
      } catch (error) {
        return NextResponse.json({ error: error instanceof Error ? error.message : "The linked PO could not cover this booking cost.", code: "PO_OVERRUN" }, { status: 409 });
      }
    }
  }
  ...
  if (projection && booking.episodeId && !budgetLine) {
    ...
    const [created] = await db.insert(budgetLines).values({ organizationId, showId: episode.showId, seasonId: episode.seasonId, episodeId: booking.episodeId, code: `BOOKING-${projection.category.toUpperCase().replaceAll(/[^A-Z0-9]+/g, "-")}`, category: projection.category, description: "Automatically calculated from approved room and artist time.", budgetedAmount: String(projection.budgetedAmount), actualAmount: String(projection.actualAmount), currency: projection.currency ?? "USD", costType: "internal" }).returning({ id: budgetLines.id, purchaseOrderId: budgetLines.purchaseOrderId, actualAmount: budgetLines.actualAmount });
    budgetLine = created;
  }
  ...
  if (budgetLine?.purchaseOrderId) await reconcilePurchaseOrder(organizationId, budgetLine.purchaseOrderId, { actorUserId: context.userId, action: "allocation.booking_actual", amount: projection?.actualAmount, metadata: { bookingId: booking.id, episodeId: booking.episodeId, category: projection?.category } });
  await writeAuditEvent({ organizationId, actorUserId: context.userId, action: isBudgetOverrun ? "booking.time_overrun_approved" : "booking.time_approved", entityType: "booking", entityId: booking.id, metadata: { episodeId: booking.episodeId, bookingCost: projection, linkedPurchaseOrderId: budgetLine?.purchaseOrderId ?? null } });
