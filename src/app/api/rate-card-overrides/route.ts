...
export async function POST(request: Request) {
  ...
  const filters = scope.type === "network" ? eq(rateCards.network, scope.network) : scope.type === "show" ? eq(rateCards.showId, scope.showId) : eq(rateCards.episodeId, scope.episodeId);
  let [card] = await db.select().from(rateCards).where(and(eq(rateCards.organizationId, organizationId), filters)).limit(1);
  if (!card) {
    const target = scope.type === "network" ? { network: scope.network, name: `${scope.network} network rate card` } : scope.type === "show" ? { showId: scope.showId, name: "Show rate card" } : { episodeId: scope.episodeId, name: "Episode rate card" };
...
export async function GET(request: Request) {
  ...
  if (!target) return NextResponse.json({ error: "Invalid rate-card scope." }, { status: 400 });
  const scopeFilter = type === "network" ? eq(rateCards.network, target) : type === "show" ? eq(rateCards.showId, target) : type === "episode" ? eq(rateCards.episodeId, target) : null;
  ...
  const own = cards.filter((card) => type === "network" ? card.network === target && !card.showId && !card.episodeId : type === "show" ? card.showId === target && !card.episodeId : card.episodeId === target);
  const chain = [own, ...(type === "episode" ? [cards.filter((card) => card.showId === showId && !card.episodeId)] : []), ...((network ? [cards.filter((card) => card.network === network && !card.showId && !card.episodeId)] : [])), ...((clientCompanyId ? [cards.filter((card) => card.clientCompanyId === clientCompanyId && !card.network && !card.showId && !card.episodeId)] : []))].flat();
