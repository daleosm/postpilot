...
export async function POST(request: Request) {
  ...
  const sourceBookings = await db.select().from(bookings).where(and(eq(bookings.organizationId, organizationId), eq(bookings.episodeId, source.id))).orderBy(asc(bookings.startsAt));
