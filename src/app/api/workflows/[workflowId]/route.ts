...
export async function PATCH(request: Request, { params }: { params: Promise<{ workflowId: string }> }) {
  ...
  if (parsed.data.stages.some((stage) => !stageIds.has(stage.id)) || parsed.data.rules.some((rule) => !stageIds.has(rule.workflowStageId))) return NextResponse.json({ error: "Workflow contains an invalid stage." }, { status: 400 });
  await db.transaction(async (tx) => {
    ...
    if (parsed.data.rules.length) await tx.insert(workflowStageApprovalRules).values(parsed.data.rules.map((rule) => ({ organizationId, workflowStageId: rule.workflowStageId, approverRole: rule.approverRole, label: rule.label, approvalOrder: rule.approvalOrder, isRequired: rule.isRequired })));
  });
  await writeAuditEvent({ organizationId, actorUserId: context.userId, action: "workflow.template_updated", entityType: "workflow", entityId: workflowId, metadata: { stageCount: parsed.data.stages.length, ruleCount: parsed.data.rules.length } });
