...
import { budgetLines, episodes, postWorkOrders, seasons, shows } from "@/lib/db/schema";
...
export async function POST(request: Request, { params }: { params: Promise<{ workOrderId: string }> }) {
  ...
  if (isDebugDemoMode) return NextResponse.json({ id: "demo-work-order-charge", debug: true }, { status: 201 });
  ...
  const [workOrder] = await db.select({
    ...
    estimatedAmount: postWorkOrders.estimatedAmount, currency: postWorkOrders.currency, showId: shows.id, seasonId: seasons.id,
  }).from(postWorkOrders)
  ...
  await writeAuditEvent({ organizationId, actorUserId: context.userId, action: "work_order.charge_posted", entityType: "post_work_order", entityId: workOrder.id, metadata: { episodeId: workOrder.episodeId, budgetLineId: line.id, actualAmount: parsed.data.actualAmount } });
