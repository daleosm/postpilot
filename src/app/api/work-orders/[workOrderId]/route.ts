...
export async function PATCH(request: Request, { params }: { params: Promise<{ workOrderId: string }> }) {
  ...
  const managerFields = ["title", "description", "department", "assigneePersonId", "assigneeRole", "vendorCompanyId", "priority", "isBlocking", "billingScope", "estimatedAmount", "currency", "clientQuoteAmount", "clientQuoteCurrency", "billingNotes", "externalUrl", "dueAt"];
  ...
  const commercialFields = ["billingScope", "estimatedAmount", "currency", "clientQuoteAmount", "clientQuoteCurrency", "billingNotes"];
  ...
  if (workOrder[0].billingStatus === "posted" && ["billingScope", "estimatedAmount", "currency", "clientQuoteAmount", "clientQuoteCurrency", "billingNotes"].some((field) => field in parsed.data)) return NextResponse.json({ error: "A charge already posted to budget cannot be changed here." }, { status: 409 });
