...
export async function POST(request: Request, { params }: { params: Promise<{ workOrderId: string }> }) {
  ...
  if (workOrder.bookingId) return NextResponse.json({ error: "This work order already has a calendar booking." }, { status: 409 });
  ...
  const [booking] = await db.transaction(async (tx) => {
    ...
    await tx.update(postWorkOrders).set({ bookingId: created.id, updatedAt: new Date() })
      .where(and(eq(postWorkOrders.id, workOrder.id), eq(postWorkOrders.organizationId, organizationId), isNull(postWorkOrders.bookingId)));
    return [created];
  });
  await Promise.all([
