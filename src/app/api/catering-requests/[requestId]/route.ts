...
export async function PATCH(request: Request, { params }: { params: Promise<{ requestId: string }> }) {
  ...
  if (parsed.data.actualCost !== undefined && parsed.data.actualCost !== null) {
    ...
    const amount = String(parsed.data.actualCost); const currency = context.organization.currency;
  ...
  await db.update(cateringRequests).set({ status: parsed.data.status, fulfilledByPersonId: runner?.id ?? null, fulfilledAt: parsed.data.status === "delivered" ? new Date() : null, actualCost: parsed.data.actualCost === undefined ? undefined : parsed.data.actualCost === null ? null : String(parsed.data.actualCost), billedAmount, markupPercent: parsed.data.actualCost === undefined ? undefined : String(markupPercent), currency: context.organization.currency, receiptReference: parsed.data.receiptReference === undefined ? undefined : parsed.data.receiptReference, billableId, budgetLineId, updatedAt: new Date() }).where(and(eq(cateringRequests.id, requestId), eq(cateringRequests.organizationId, context.organization.organizationId)));
