...
function Clearance({ status }: { status: string }) { const colors: Record<string, string> = { clear: "bg-[#e5eee8] text-[#477263]", po_required: "bg-[#f6ebde] text-[#9a613f]", finance_approval: "bg-[#f4e5df] text-[#9a5b3f]", on_hold: "bg-[#f1ece9] text-[#8c5f51]" }; return <span className={`rounded px-2 py-0.5 text-[10px] font-semibold ${colors[status] ?? colors.clear}`}>{status === "clear" ? "Clear to book" : status.replaceAll("_", " ")}</span>; }
