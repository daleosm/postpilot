...
export default async function CrmAccountPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  if (!mayManageShows && !mayManageBudget) redirect("/");
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,0.9fr)]">
      <Panel title="Account profile" icon={<UserRound size={16}/> }>
        <div className="grid divide-y divide-[#efeeea] sm:grid-cols-2 sm:divide-x sm:divide-y-0"><ProfileItem label="Account owner" value={owner ? `${owner.name} · ${owner.role.replaceAll("_", " ")}` : "Unassigned"}/><ProfileItem label="Booking clearance" value={company.bookingClearance.replaceAll("_", " ")}/><ProfileItem label="Service category" value={company.serviceCategory ?? (isVendor ? "Not set" : "Not applicable")}/>{mayManageBudget ? <ProfileItem label="Finance email" value={company.financeEmail ?? company.billingEmail ?? "Not set"}/> : <ProfileItem label="Supplier status" value={isVendor ? (company.isPreferredSupplier ? "Preferred supplier" : "Approved supplier") : "—"}/>}</div>
      </Panel>
    ...
    {mayManageBudget && <section className="grid gap-4 xl:grid-cols-2">
...
function Clearance({ status }: { status: string }) { const colors: Record<string, string> = { clear: "bg-[#e5eee8] text-[#477263]", authorisation_required: "bg-[#f6ebde] text-[#9a613f]", finance_approval: "bg-[#f4e5df] text-[#9a5b3f]", on_hold: "bg-[#f1ece9] text-[#8c5f51]" }; return <span className={`rounded px-2 py-0.5 text-[10px] font-semibold ${colors[status] ?? colors.clear}`}>{status === "clear" ? "Clear to book" : status.replaceAll("_", " ")}</span>; }
