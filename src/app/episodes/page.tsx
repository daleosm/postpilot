...
async function getEpisodesData(): Promise<{ episodes: EpisodeTableRow[]; seasons: EpisodeSeason[]; people: Array<{ id: string; name: string; role: string }> }> {
  ...
  const people = await listTeam(context.organization.organizationId); return { episodes: rows.map((episode) => ({ id: episode.id, code: episode.productionCode, title: episode.title, number: episode.number, showId: episode.showId, showTitle: episode.showTitle, seasonNumber: episode.seasonNumber, workflowStage: episode.workflowStage, editorName: episode.editorName, producerName: episode.producerName, lockedCutDate: episode.lockedCutDate, deliveryDeadline: episode.deliveryDeadline, qcStatus: episode.qcStatus, status: episode.status })), seasons: shows.flatMap((show) => show.seasons.map((season) => ({ id: season.id, label: `${show.title} · Season ${season.number}` }))), people };
}
