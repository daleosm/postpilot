...
export default async function EpisodeDetailPage({ params }: { params: Promise<{ episodeId: string }> }) {
  ...
  if (!data) notFound();
  const { episode } = data;
  const [canManageWorkOrders, canUpdateWorkOrders, canManageCommercial, canManageShows] = await Promise.all([can("manage_work_orders"), can("update_assigned_work"), can("manage_budget"), can("manage_shows")]);
...
async function getEpisodeDetail(episodeId: string) {
  if (isDebugDemoMode) {
    ...
    return { episode: details, schedule: demo.schedule.filter((item) => item.episodeTitle === episode.title), budget: [{ id: "demo-budget", category: "Editorial + finishing", actualAmount: "42150.00", budgetedAmount: "48000.00" }], activity: demo.dashboard.activity, workflowStages: [], workflowApprovalRules: [], workflowApprovals: [], workflowApprovers, episodeTeam: [], workOrders: [] };
  }
