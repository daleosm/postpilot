...
async function getEpisodeDetail(episodeId: string) {
  if (isDebugDemoMode) {
    ...
    return { episode: details, schedule: demo.schedule.filter((item) => item.episodeTitle === episode.title), budget: [{ id: "demo-budget", category: "Editorial + finishing", actualAmount: "42150.00", budgetedAmount: "48000.00" }], activity: demo.dashboard.activity.filter((item) => item.entityId === episode.id), workflowStages: [], workflowApprovalRules: [], workflowApprovals: [], workflowApprovers, episodeTeam: [], workOrders: [], qcHistory: [], qcIssueHistory: [] };
  }
