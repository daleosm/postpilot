...
export default async function EpisodeDetailPage({ params }: { params: Promise<{ episodeId: string }> }) {
  ...
  if (!data) notFound();
  const { episode } = data;
  const [canManageWorkOrders, canUpdateWorkOrders, canManageCommercial, canManageShows, canManageQc, canVerifyQc, canWaiveQc, canApproveReviews, organizationContext] = await Promise.all([can("manage_work_orders"), can("update_assigned_work"), can("manage_budget"), can("manage_shows"), can("manage_qc"), can("verify_qc"), can("waive_qc"), can("approve_reviews"), getActiveOrganizationContext()]);
