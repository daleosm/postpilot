...
export default async function EpisodeDetailPage({ params }: { params: Promise<{ episodeId: string }> }) {
  ...
  if (!data) notFound();
  const assignedToEpisode = !isDebugDemoMode && await isAssignedToEpisode(episodeId);
  const currentPersonId = organizationContext?.person?.id ?? null;
  if (!isDebugDemoMode && organizationContext?.organization?.role === "client") {
    let sharedManifest: Awaited<ReturnType<typeof getActiveSharedDeliveryManifest>> | null = null;
    try {
      sharedManifest = await getActiveSharedDeliveryManifest(episodeId);
    } catch (error) {
      ...
      if (!(error instanceof DeliveryManifestError) || error.status >= 500) throw error;
  ...
  if (!isDebugDemoMode && !canSeeAllEpisodes && !assignedToEpisode) notFound();
  const { episode } = data;
