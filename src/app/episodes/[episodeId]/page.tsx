...
async function getEpisodeDetail(episodeId: string) {
  if (isDebugDemoMode) {
    ...
    const details = { ...episode, workflowStageId: null, workflowStage: null, editorName: episode.showTitle === "Under Current" ? "Leah Morgan" : "James Liu", producerName: episode.showTitle === "Under Current" ? "Noah Chen" : "Maya Ortiz", lockedCutDate: null };
    ...
    return { episode: details, schedule: demo.schedule.filter((item) => item.episodeTitle === episode.title), budget: [{ id: "demo-budget", category: "Editorial + finishing", actualAmount: "42150.00", budgetedAmount: "48000.00" }], activity: demo.dashboard.activity, workflowStages: [], workflowApprovalRules: [], workflowApprovals: [], workflowApprovers, workOrders: [] };
  }
