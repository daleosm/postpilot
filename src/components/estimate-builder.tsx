...
function EstimateRow({ row, resources, onChange, onResolve, onRemove }: { row: Row; resources: ResourceOptions; onChange: (patch: Partial<Row>) => void; onResolve: () => void; onRemove: () => void }) {
  ...
  return <div className="grid gap-2 rounded-xl border border-[#e4e6e2] bg-white p-3 lg:grid-cols-[130px_118px_minmax(180px,1fr)_100px_90px_120px_105px_40px] lg:items-center lg:rounded-none lg:border-x-0 lg:border-t-0 lg:px-0"><select value={row.department} onChange={(event) => onChange({ department: event.target.value })} className="control text-xs">{departments.map((department) => <option key={department}>{department}</option>)}</select><select value={row.type} onChange={(event) => onChange({ type: event.target.value as ItemType, resourceId: "", preview: undefined })} className="control text-xs"><option value="service">Service</option><option value="room">Room</option><option value="person">Person</option><option value="vendor">External vendor</option><option value="fixed">Fixed cost</option></select><div className="min-w-0">{row.type === "service" ? <select value={row.resourceId} onChange={(event) => choose(`service:${event.target.value}`)} className="control text-xs"><option value="">Choose service</option>{resources.services.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : row.type === "room" ? <><select value={row.resourceId} onChange={(event) => choose(`room:${event.target.value}`)} className="control text-xs"><option value="">Choose room</option>{resources.rooms.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input value={row.category} onChange={(event) => onChange({ category: event.target.value })} placeholder="Rate category" className="control mt-2 text-xs" /></> : row.type === "person" ? <><select value={row.resourceId} onChange={(event) => choose(`person:${event.target.value}`)} className="control text-xs"><option value="">Choose person</option>{resources.people.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input value={row.category} onChange={(event) => onChange({ category: event.target.value })} placeholder="Rate category" className="control mt-2 text-xs" /></> : row.type === "vendor" ? <><select value={row.resourceId} onChange={(event) => choose(`vendor:${event.target.value}`)} className="control text-xs"><option value="">Choose vendor</option>{resources.vendors.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input value={row.category} onChange={(event) => onChange({ category: event.target.value })} placeholder="Cost category" className="control mt-2 text-xs" /></> : <input value={row.category} onChange={(event) => onChange({ category: event.target.value })} placeholder="Fixed-cost category" className="control text-xs" />}</div><input type="number" min="0.01" step="0.01" value={row.quantity} onChange={(event) => onChange({ quantity: event.target.value })} className="control text-xs" /><select value={row.unit} onChange={(event) => onChange({ unit: event.target.value })} className="control text-xs">{units.map((unit) => <option key={unit}>{unit}</option>)}</select><div className="text-xs">{needsManualRate || row.manualRate ? <><input type="number" min="0" step="0.01" value={row.manualRate} onChange={(event) => onChange({ manualRate: event.target.value })} placeholder="Rate" className="control text-xs" /><input value={row.overrideReason} onChange={(event) => onChange({ overrideReason: event.target.value })} placeholder={needsManualRate ? "Quote/source (optional)" : "Override reason"} className="control mt-2 text-xs" /></> : <button type="button" onClick={onResolve} className="text-left font-medium text-[#4c7568] hover:underline">{row.preview ? sourceLabel(row.preview.rate_source) : "Resolve rate"}</button>}</div><div className="text-sm font-semibold text-[#303a36]">{row.preview ? <>{formatMoney(row.preview.estimate, row.preview.currency)}<p className="mt-1 text-[10px] font-normal text-[#858a87]">{row.preview.quantity} × {formatMoney(row.preview.rate, row.preview.currency)}</p></> : "—"}</div><Button isIconOnly variant="tertiary" aria-label="Remove estimate item" onPress={onRemove}><Trash2 size={16} /></Button>{row.error && <p className="lg:col-span-8 text-xs text-[#ad5e43]">{row.error}</p>}<input value={row.description} onChange={(event) => onChange({ description: event.target.value })} placeholder="Optional operational note" className="control lg:col-span-3 text-xs" /></div>;
}
