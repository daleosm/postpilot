...
const newRow = (): Row => ({ id: crypto.randomUUID(), department: "Editorial", type: "service", resourceId: "", category: "", description: "", quantity: "1", unit: "", manualRate: "", overrideReason: "" });
...
export function EstimateBuilder({ episode, resources }: { episode: { id: string; label: string }; resources: ResourceOptions }) {
  ...
  const update = useCallback((id: string, patch: Partial<Row>) => setRows((current) => current.map((row) => {
    ...
    {open && <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 p-0 backdrop-blur-[2px] sm:items-center sm:p-5">
      <section role="dialog" aria-modal="true" aria-label="Build episode estimate" className="flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-t-2xl bg-[#fefefa] shadow-2xl sm:rounded-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-7">
          ...
          <div className="space-y-3 pt-3">{rows.map((row) => <EstimateRow key={row.id} episodeId={episode.id} row={row} resources={resources} onChange={(patch) => update(row.id, patch)} onResolve={() => resolve(row)} onRemove={() => setRows((items) => items.length === 1 ? items : items.filter((item) => item.id !== row.id))} />)}</div>
...
function ConfiguredUnitSelect({ episodeId, row, onChange }: { episodeId: string; row: Row; onChange: (patch: Partial<Row>) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const params = new URLSearchParams({ episode_id: episodeId, category: row.category, rate_resource_type: row.type, rate_resource_id: row.resourceId });
      const response = await postpilotApiFetch<{ units: Array<{ unit: string; label: string }> }>(`/budget/estimate-available-units?${params.toString()}`);
  ...
  }, [canResolve, episodeId, row.category, row.resourceId, row.type]);
  useEffect(() => {
    const timer = window.setTimeout(() => { void load(); }, 0);
    return () => window.clearTimeout(timer);
  }, [load]);

  return <select aria-label="Estimate billing unit" value={row.unit} onChange={(event) => onChange({ unit: event.target.value })} disabled={!canResolve || loading || !availableUnits.length} className="control text-xs"><option value="">{loading ? "Loading prices…" : !canResolve ? "Choose resource" : availableUnits.length ? "Choose unit" : "No configured price"}</option>{availableUnits.map(({ unit, label }) => <option key={unit} value={unit}>{label}</option>)}</select>;
}
