...
export function EpisodeWorkOrders({ episodeId, initialWorkOrders, people, stages, currentStageId, canManage, canUpdate, canManageCommercial }: { episodeId: string; initialWorkOrders: WorkOrder[]; people: Person[]; stages: readonly { id: string; name: string; position: number }[]; currentStageId: string | null; canManage: boolean; canUpdate: boolean; canManageCommercial: boolean }) {
  const router = useRouter();
  const [workOrders, setWorkOrders] = useState(initialWorkOrders);
  const [open, setOpen] = useState(false);
  const [message, setMessage] = useState("");
  const form = useForm<WorkOrderInput, unknown, WorkOrderValues>({ resolver: zodResolver(workOrderSchema), defaultValues: { workflowStageId: currentStageId ?? "", title: "", description: "", department: "", assigneePersonId: "", assigneeRole: "", priority: "normal", isBlocking: Boolean(currentStageId), billingScope: "included", estimatedAmount: "", currency: "USD", billingNotes: "", externalUrl: "", dueAt: "" } });
  ...
  async function create(values: WorkOrderValues) {
    setMessage("");
    const { estimatedAmount, currency, billingNotes, ...operationalValues } = values;
    const commercialValues = canManageCommercial ? { estimatedAmount: estimatedAmount === "" ? null : estimatedAmount, currency, billingNotes: billingNotes || null } : {};
    const response = await fetch("/api/work-orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...operationalValues, ...commercialValues, episodeId, workflowStageId: values.workflowStageId || null, assigneePersonId: values.assigneePersonId || null, assigneeRole: values.assigneeRole || null, description: values.description || null, department: values.department || null, externalUrl: values.externalUrl || null, dueAt: values.dueAt || null }) });
    const body = await response.json().catch(() => null);
    if (!response.ok) { setMessage(body?.error ?? "Could not create the work order."); return; }
    setOpen(false); form.reset({ workflowStageId: currentStageId ?? "", title: "", description: "", department: "", assigneePersonId: "", assigneeRole: "", priority: "normal", isBlocking: Boolean(currentStageId), billingScope: "included", estimatedAmount: "", currency: "USD", billingNotes: "", externalUrl: "", dueAt: "" }); setMessage("Work order created."); router.refresh();
  }
