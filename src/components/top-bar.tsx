...
export function TopBar({ debugUser, debugUsers, debugMode, activeOrganization, organizations, shows, activeShow }: TopBarProps) {
  ...
  async function chooseOrganization(organization: OrganizationMembership) {
    ...
    try {
      const result = await postpilotApiFetch<{ redirect_to?: string }>("/organizations/active", {
        method: "POST",
        body: { organization_id: organization.organizationId, pathname },
      });
  ...
  async function chooseDebugUser(user: DebugUser) {
    if (switchingDebugUserId) return;
    setDebugError(null);
    setSwitchingDebugUserId(user.userId);
    try {
      const result = await postpilotApiFetch<{ redirect_to?: string }>("/debug/user", {
        method: "PUT",
        body: { user_id: user.userId, pathname },
      });
      ...
      setDebugError(error instanceof Error ? error.message : "Could not switch debug user.");
    } finally {
      setSwitchingDebugUserId(null);
    }
  }
  ...
  return (
    <header className="pp-topbar sticky top-0 z-40 flex h-[60px] items-center justify-between gap-2 border-b px-3 backdrop-blur-xl sm:gap-3 sm:px-6 lg:h-[66px] lg:px-8">
      ...
      <div className="flex shrink-0 items-center gap-2.5">
        {hasDebugControls && <div className="relative" ref={userRef}><Button variant="tertiary" onClick={() => { setDebugError(null); setUserOpen((value) => !value); }} aria-label="Switch debug user" isDisabled={Boolean(switchingDebugUserId)} className="flex h-8 w-[76px] min-w-0 max-w-[76px] gap-1.5 border border-[#e6c98a] bg-[#fff9ed] px-2 text-xs text-[#765720] hover:bg-[#fff2d6] sm:w-auto sm:max-w-[190px]"><UserRound size={14} className="shrink-0" /><span className="font-semibold sm:hidden">User</span><span className="hidden font-semibold uppercase tracking-[.08em] sm:inline">Debug</span><span className="hidden truncate sm:inline">{activeDebugUser.name}</span><ChevronDown size={13} className="shrink-0" /></Button>{userOpen && <div className="absolute right-0 top-10 z-30 w-60 overflow-hidden rounded-lg border border-[#e1c986] bg-[#fffdf8] p-1 shadow-lg"><p className="px-3 py-2 text-[10px] font-semibold uppercase tracking-[.08em] text-[#8b6721]">Debug user / role</p><p className="px-3 pb-2 text-[10px] leading-4 text-[#8a7a58]">All seeded users are available. Maya is the platform admin across tenants.</p><div className="max-h-[calc(100vh-9rem)] overflow-y-auto overscroll-contain">{tenantDebugUsers.map((user) => <Button key={user.id} variant="tertiary" onClick={() => chooseDebugUser(user)} isDisabled={Boolean(switchingDebugUserId)} className="flex h-auto w-full justify-between rounded-md px-3 py-2 text-left text-xs text-[#4f5753] hover:bg-[#fff3dc]"><span><span className="block font-medium">{user.name}</span><span className="block text-[10px] text-[#898e8a]">{user.label}</span></span>{switchingDebugUserId === user.userId ? <LoaderCircle size={14} className="shrink-0 animate-spin text-[#8a6727]" /> : activeDebugUser.userId === user.userId && <Check size={14} className="text-[#8a6727]" />}</Button>)}{!tenantDebugUsers.length && <p className="px-3 py-4 text-xs text-[#8a7a58]">No tenant users are available.</p>}</div></div>}{debugError && <p className="absolute right-0 top-10 z-40 mt-1 w-64 rounded-md border border-[#e7c5bd] bg-[#fff8f5] px-2.5 py-2 text-[11px] leading-4 text-[#a44e3b] shadow-sm" role="alert">{debugError}</p>}</div>}
        ...
        <Button variant="tertiary" size="sm" onPress={() => router.push("/settings/sso")} className="hidden h-8 gap-1.5 px-2 text-xs text-[#626865] hover:bg-[#f0f0ed] sm:flex">
