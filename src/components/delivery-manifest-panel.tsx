...
function RiskBadge({ risk }: { risk: "on_track" | "at_risk" | "overdue" }) { const tone = risk === "overdue" ? "pp-status--danger" : risk === "at_risk" ? "pp-status--warning" : "pp-status--success"; return <span className={`pp-status ${tone}`}>{risk.replaceAll("_", " ")}</span>; }
function StatusBadge({ status }: { status: string }) { const tone = status === "receipt_confirmed" || status === "waived" ? "pp-status--success" : ["qc_failed", "rejected"].includes(status) ? "pp-status--danger" : status === "dispatched" ? "pp-status--info" : "pp-status--neutral"; return <span className={`pp-status ${tone}`}>{status.replaceAll("_", " ")}</span>; }
