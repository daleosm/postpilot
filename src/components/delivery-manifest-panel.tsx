...
export function DeliveryManifestPanel({ episodeId, manifest, profiles, canManageManifest, canUpdate, canConfirmReceipt }: { episodeId: string; manifest: DeliveryManifest | null; profiles: Array<{ id: string; name: string }>; canManageManifest: boolean; canUpdate: boolean; canConfirmReceipt: boolean }) {
  ...
  return <div className="space-y-5">
    <section className="rounded-xl border border-[#e4e7e3] bg-[#fafbf9] p-4">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-[#77817d]">Delivery manifest</p><h2 className="mt-1 text-lg font-semibold text-[#303936]">{manifest.profileName}</h2><p className="mt-1 text-xs text-[#727b76]">{readiness.requiredItemCount} required · {optionalCount} optional · {readiness.completedRequiredItemCount} confirmed</p></div><RiskBadge risk={readiness.deadlineRisk} /></div>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-3"><Metric label="Progress" value={`${readiness.progressPercent}%`} /><Metric label="Facility dispatch" value={readiness.facilityDispatched ? "Complete" : `${readiness.outstandingRequiredItemCount} outstanding`} tone={readiness.facilityDispatched ? "good" : undefined} /><Metric label="Receipt" value={readiness.clientNetworkAccepted ? "Confirmed" : "Awaiting recipient"} tone={readiness.clientNetworkAccepted ? "good" : undefined} /></div>
...
function DeliveryItemRow({ episodeId, item, canUpdate, canConfirmReceipt }: { episodeId: string; item: DeliveryItem; canUpdate: boolean; canConfirmReceipt: boolean }) {
  ...
  return <div className="p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-medium text-[#404a45]">{item.label}</p>{item.required ? <span className="rounded-full bg-[#e7eee8] px-2 py-0.5 text-[10px] font-semibold text-[#4d7063]">Required</span> : <span className="rounded-full bg-[#eff1ef] px-2 py-0.5 text-[10px] font-semibold text-[#6f7873]">Optional</span>}</div><p className="mt-1 text-xs text-[#77817c]">{details || item.componentType}</p></div><StatusBadge status={item.status} /></div>
    <div className="mt-3 grid gap-2 text-xs text-[#69746f] sm:grid-cols-2 lg:grid-cols-4"><Detail label="Due" value={formatDate(item.dueDate)} /><Detail label="Recipient" value={item.recipientName ?? (item.requiresExternalRecipient ? "Required before dispatch" : "Not specified")} /><Detail label="QC" value={item.qcRequired ? item.qcResult.replaceAll("_", " ") : "Not required"} /><Detail label="Receipt" value={item.receiptConfirmedAt ? `Confirmed ${formatDate(item.receiptConfirmedAt)}` : "Awaiting"} /></div>
    {(item.externalReference || item.externalUrl) && <div className="mt-3 text-xs">{item.externalUrl ? <a className="inline-flex items-center gap-1 font-semibold text-[#47756a] hover:underline" href={item.externalUrl} target="_blank" rel="noreferrer">{item.externalReference || "Open external reference"} <ExternalLink size={12} /></a> : <span className="font-medium text-[#60706a]">Reference: {item.externalReference}</span>}</div>}
