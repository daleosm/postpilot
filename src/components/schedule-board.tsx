...
export function ScheduleBoard({ bookings, rooms, resources, cateringRequests, workOrders, initialDate, initialWorkOrderId, initialBookingId, canManage, canSubmitOwnTime, currentPersonId }: { bookings: ScheduleBooking[]; rooms: Array<{ id: string; name: string; type: string }>; resources: BookingResources; cateringRequests: CateringRequest[]; workOrders: Array<SchedulableWorkOrder & { bookingId: string | null; workType: string; assigneePersonId: string | null; status: string }>; initialDate: string; initialWorkOrderId: string | null; initialBookingId: string | null; canManage: boolean; canSubmitOwnTime: boolean; currentPersonId: string | null }) {
  ...
  return <div className="space-y-4">
    ...
    {selectedBooking && <div className="fixed bottom-5 right-5 z-40 max-w-[calc(100vw-2.5rem)] rounded-lg border border-[#e2e3de] bg-[#fafbf9] p-3 shadow-lg"><div className="mb-2 text-xs text-[#67716c]"><p className="font-semibold text-[#404a45]">{selectedBooking.title}</p>{selectedBooking.budgetItem && <p className="mt-1">Budget item · {selectedBooking.budgetItem.label}{selectedBooking.budgetItemContext ? ` · ${money(selectedBooking.budgetItemContext.remainingEstimate, selectedBooking.budgetItemContext.currency)} remaining estimate` : ""}</p>}{selectedBooking.actualStartsAt && selectedBooking.actualBudgetStatus === "unallocated" && <p className="mt-1 font-medium text-[#a35e41]">Actual time is unallocated.</p>}</div><div className="flex flex-wrap gap-2">{canManage && <BookingFormDialog key={selectedBooking.id} resources={resources} initialStart={toInput(cursor)} booking={selectedBooking} onClose={() => setSelectedBooking(null)} />}{canSubmitOwnTime && selectedBooking.personId === currentPersonId && <><ActualTimeDialog booking={selectedBooking} /><BookingConflictFlagDialog bookingId={selectedBooking.id} title={selectedBooking.title} /></>}<Button size="sm" variant="tertiary" onPress={() => setSelectedBooking(null)}>Close</Button></div></div>}
