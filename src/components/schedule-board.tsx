...
function GanttBookingBar({ placement, totalMinutes, onSelect }: { placement: GanttBooking; totalMinutes: number; onSelect: (booking: ScheduleBooking) => void }) {
  ...
  return <button data-testid={`booking-bar-${booking.id}`} type="button" onClick={() => onSelect(booking)} aria-label={`Edit ${optionLabel}${booking.title}`} title={`${booking.isOption ? `${optionLabel}provisional hold. ` : ""}Client: ${timeLabel(booking.startsAt)}–${timeLabel(booking.endsAt)}. Operational: ${timeLabel(operationalStart(booking))}–${timeLabel(operationalEnd(booking))}.${booking.workflowState ? ` Episode workflow: ${booking.workflowState.primaryStageName ?? "not started"} (${booking.workflowState.displayStatus.replaceAll("_", " ")}).` : ""}`} style={{ top: `${placement.lane * 48 + 4}px`, left: `calc(${(placement.start / totalMinutes) * 100}% + 3px)`, width: `calc(${Math.max(1.5, ((placement.end - placement.start) / totalMinutes) * 100)}% - 6px)` }} className={`absolute ${booking.isOption ? "h-7 border-dashed py-1" : "h-10 py-1.5"} overflow-hidden rounded-md border-l-[3px] px-2 text-left shadow-sm transition-shadow hover:z-10 hover:shadow-md focus:z-10 focus:outline-none focus:ring-2 focus:ring-[#66877f] ${booking.isOption ? "border-[#b99143] bg-[#fff9e9]" : bookingColors(booking.bookingType)}`}><p className="truncate text-[11px] font-semibold text-[#414945]">{optionLabel}{booking.title}</p>{!booking.isOption && <p className="mt-0.5 truncate text-[10px] text-[#68716d]">{booking.actualStartsAt ? `Actual ${timeLabel(booking.actualStartsAt)}–${timeLabel(booking.actualEndsAt ? booking.actualEndsAt : booking.endsAt)}` : `Client ${timeLabel(booking.startsAt)}–${timeLabel(booking.endsAt)}`} · {booking.workflowState?.primaryStageName ?? booking.personName ?? "Unassigned"}</p>}</button>;
}
