...
import { CommercialTreatmentBadge, CommercialTreatmentSummary, commercialTreatmentDetails, type CommercialTreatment } from "@/components/commercial-treatment";
...
export type ScheduleBooking = {
  ...
  status: string;
  bookingType: string;
  commercialTreatment: CommercialTreatment | null;
  roomId: string | null;
  ...
  actualBudgetStatus: "not_submitted" | "allocated" | "unallocated";
  budgetItemLabel: string | null;
  budgetItem: { id: string; label: string } | null;
  budgetItemContext: { estimatedAmount: number; actualAmount: number; remainingEstimate: number; currency: string } | null;
...
export function ScheduleBoard({ bookings, rooms, resources, cateringRequests, workOrders, initialDate, initialWorkOrderId, initialBookingId, canManage, canSubmitOwnTime, currentPersonId }: { bookings: ScheduleBooking[]; rooms: Array<{ id: string; name: string; type: string }>; resources: BookingResources; cateringRequests: CateringRequest[]; workOrders: Array<SchedulableWorkOrder & { bookingId: string | null; workType: string; assigneePersonId: string | null; status: string }>; initialDate: string; initialWorkOrderId: string | null; initialBookingId: string | null; canManage: boolean; canSubmitOwnTime: boolean; currentPersonId: string | null }) {
  ...
  return <div className="space-y-4">
    ...
    {selectedBooking && <div className="fixed bottom-5 right-5 z-40 max-w-[calc(100vw-2.5rem)] rounded-lg border border-[#e2e3de] bg-[#fafbf9] p-3 shadow-lg"><div className="mb-2 text-xs text-[#67716c]"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-[#404a45]">{selectedBooking.title}</p><CommercialTreatmentBadge treatment={selectedBooking.commercialTreatment} /></div><CommercialTreatmentSummary treatment={selectedBooking.commercialTreatment} className="mt-1 max-w-sm" />{selectedBooking.budgetItem && <p className="mt-1">Budget item · {selectedBooking.budgetItem.label}{selectedBooking.budgetItemContext ? ` · ${money(selectedBooking.budgetItemContext.remainingEstimate, selectedBooking.budgetItemContext.currency)} remaining estimate` : ""}</p>}{selectedBooking.actualStartsAt && selectedBooking.actualBudgetStatus === "unallocated" && <p className="mt-1 font-medium text-[#a35e41]">Actual time is unallocated.</p>}</div><div className="flex flex-wrap gap-2">{canManage && <BookingFormDialog key={selectedBooking.id} resources={resources} initialStart={toInput(cursor)} booking={{ ...selectedBooking, commercialTreatment: selectedBooking.commercialTreatment ?? undefined }} onClose={() => setSelectedBooking(null)} />}{canSubmitOwnTime && selectedBooking.personId === currentPersonId && <><ActualTimeDialog booking={selectedBooking} /><BookingConflictFlagDialog bookingId={selectedBooking.id} title={selectedBooking.title} /></>}<Button size="sm" variant="tertiary" onPress={() => setSelectedBooking(null)}>Close</Button></div></div>}
...
function GanttBookingBar({ placement, totalMinutes, hasConflict, onSelect }: { placement: GanttBooking; totalMinutes: number; hasConflict: boolean; onSelect: (booking: ScheduleBooking) => void }) {
  ...
  const budgetNote = booking.actualStartsAt && booking.actualBudgetStatus === "unallocated" ? " · Unallocated actual" : "";
  return <button data-testid={`booking-bar-${booking.id}`} data-booking-state={state} type="button" onClick={() => onSelect(booking)} aria-label={`Edit ${optionLabel}${booking.title}`} title={`${bookingStateLabel(state)}. ${commercialTreatmentDetails(booking.commercialTreatment).label}: ${commercialTreatmentDetails(booking.commercialTreatment).summary} ${booking.isOption ? `${optionLabel}provisional hold. ` : ""}Client: ${timeLabel(booking.startsAt)}–${timeLabel(booking.endsAt)}. Operational: ${timeLabel(operationalStart(booking))}–${timeLabel(operationalEnd(booking))}.${booking.actualStartsAt ? ` Budget actual: ${booking.actualBudgetStatus.replaceAll("_", " ")}.` : ""}${booking.workflowState ? ` Episode workflow: ${booking.workflowState.primaryStageName ?? "not started"} (${booking.workflowState.displayStatus.replaceAll("_", " ")}).` : ""}`} style={{ top: `${placement.lane * 48 + 4}px`, left: `calc(${(placement.start / totalMinutes) * 100}% + 3px)`, width: `calc(${Math.max(1.5, ((placement.end - placement.start) / totalMinutes) * 100)}% - 6px)` }} className={`gantt-booking gantt-booking--${state} gantt-booking--${booking.bookingType} absolute ${state === "hold" ? "h-7 border-dashed py-1" : "h-10 py-1.5"} overflow-hidden rounded-md border-l-[3px] px-2 text-left shadow-sm transition-shadow hover:z-10 hover:shadow-md focus:z-10 focus:outline-none focus:ring-2 focus:ring-[#66877f]`}><p className="truncate text-[11px] font-semibold text-[#414945]"><span className="mr-1 text-[9px] font-bold uppercase tracking-[.06em] text-[#6d7771]">{bookingStateLabel(state)}</span>{optionLabel}{booking.title}</p>{state !== "hold" && <p className={`mt-0.5 truncate text-[10px] ${booking.actualStartsAt && booking.actualBudgetStatus === "unallocated" ? "text-[#a35e41]" : "text-[#68716d]"}`}>{booking.actualStartsAt ? `Actual ${timeLabel(booking.actualStartsAt)}–${timeLabel(booking.actualEndsAt ? booking.actualEndsAt : booking.endsAt)}` : `Client ${timeLabel(booking.startsAt)}–${timeLabel(booking.endsAt)}`} · {commercialTreatmentDetails(booking.commercialTreatment).label}{budgetNote}</p>}</button>;
}
...
function money(value: number, currency: string) { try { return new Intl.NumberFormat("en-GB", { style: "currency", currency, maximumFractionDigits: 0 }).format(value); } catch { return `${currency} ${value.toFixed(2)}`; } }
