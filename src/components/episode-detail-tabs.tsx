...
export function EpisodeDetailTabs({ data, canManageEpisodes, canManageWorkOrders, canUpdateWorkOrders, canManageCommercial, canManageQc, canVerifyQc, canWaiveQc, canApproveReviews, currentPersonId }: { data: WorkspaceData; canManageEpisodes: boolean; canManageWorkOrders: boolean; canUpdateWorkOrders: boolean; canManageCommercial: boolean; canManageQc: boolean; canVerifyQc: boolean; canWaiveQc: boolean; canApproveReviews: boolean; currentPersonId: string | null }) {
  ...
  return (
    <section className="panel overflow-hidden">
      ...
      <div className="p-5"><TabContent tab={tab} data={data} canManageEpisodes={canManageEpisodes} canManageWorkOrders={canManageWorkOrders} canUpdateWorkOrders={canUpdateWorkOrders} canManageCommercial={canManageCommercial} canManageQc={canManageQc} canVerifyQc={canVerifyQc} canWaiveQc={canWaiveQc} canApproveReviews={canApproveReviews} currentPersonId={currentPersonId} /></div>
    </section>
  );
}

function TabContent({ tab, data, canManageEpisodes, canManageWorkOrders, canUpdateWorkOrders, canManageCommercial, canManageQc, canVerifyQc, canWaiveQc, canApproveReviews, currentPersonId }: { tab: TabName; data: WorkspaceData; canManageEpisodes: boolean; canManageWorkOrders: boolean; canUpdateWorkOrders: boolean; canManageCommercial: boolean; canManageQc: boolean; canVerifyQc: boolean; canWaiveQc: boolean; canApproveReviews: boolean; currentPersonId: string | null }) {
  if (tab === "Overview") return <EpisodeOverview data={data} />;
  if (tab === "Workflow") return <WorkflowPanel key={`${data.episode.workflowStageId ?? ""}:${data.workflowApprovals.map((approval) => `${approval.id}:${approval.status}:${approval.respondedAt ?? ""}`).join("|")}:${data.workflowTracks.map((track) => `${track.id}:${track.status}:${track.completedAt ?? ""}`).join("|")}`} episodeId={data.episode.id} initialStageId={data.episode.workflowStageId} stages={data.workflowStages} rules={data.workflowApprovalRules} approvals={data.workflowApprovals} tracks={data.workflowTracks} episodeTeam={data.episodeTeam} canManageEpisodes={canManageEpisodes} canApproveReviews={canApproveReviews} currentPersonId={currentPersonId} />;
...
function QcPanel({ episodeId, episodeStatus, initialHistory, initialIssues, workOrders, canManage, canVerify, canWaive }: { episodeId: string; episodeStatus: string; initialHistory: QcReport[]; initialIssues: QcIssue[]; workOrders: WorkOrder[]; canManage: boolean; canVerify: boolean; canWaive: boolean }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,.9fr)]">
    ...
    <section className="rounded-xl border border-[#e5e7e3] bg-[#fafbf9] p-4"><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-[#77817d]">Record QC result</p>{canManage ? <form action={submit} className="mt-3 space-y-3"><label className="block text-xs font-medium text-[#56605b]">Result<select value={status} onChange={(event) => setStatus(event.target.value)} className="mt-1.5 h-10 w-full rounded-md border border-[#dfe3df] bg-white px-2 text-sm"><option value="in_progress">QC in progress</option>{canVerify && <option value="passed">Passed — verified re-QC</option>}<option value="failed">Failed — corrections required</option>{canWaive && <option value="waived">Waived</option>}</select></label>{!canVerify && <p className="rounded-md bg-[#eef1ee] px-3 py-2 text-xs leading-5 text-[#66716c]">A team member with the <strong>Verify QC</strong> permission records the final passed re-QC result.</p>}<label className="block text-xs font-medium text-[#56605b]">External report link <span className="font-normal text-[#858c88]">(optional)</span><input name="reportUrl" type="url" placeholder="https://…" className="mt-1.5 h-10 w-full rounded-md border border-[#dfe3df] bg-white px-3 text-sm" /></label><label className="block text-xs font-medium text-[#56605b]">Summary <span className="font-normal text-[#858c88]">(optional)</span><textarea name="summary" rows={4} placeholder="Result, exceptions, and next steps…" className="mt-1.5 w-full rounded-md border border-[#dfe3df] bg-white p-3 text-sm" /></label>{status === "waived" && <label className="block text-xs font-medium text-[#56605b]">Waiver reason<textarea name="waiverReason" rows={3} required className="mt-1.5 w-full rounded-md border border-[#dfe3df] bg-white p-3 text-sm" /></label>}<Button type="submit" variant="primary" isDisabled={saving} className="bg-[#3f7563] text-white">{saving ? "Recording…" : "Record QC result"}</Button>{message && <p role="status" className={`text-xs ${message.includes("Could not") ? "text-[#a35e41]" : "text-[#3f7563]"}`}>{message}</p>}</form> : <p className="mt-3 text-sm leading-6 text-[#727b76]">You can view QC history, but your current role cannot record QC results.</p>}</section>
...
function WorkflowPanel({ episodeId, initialStageId, stages, rules, approvals, tracks, episodeTeam, canManageEpisodes, canApproveReviews, currentPersonId }: { episodeId?: string; initialStageId: string | null; stages: readonly WorkflowStage[]; rules: WorkflowApprovalRule[]; approvals: WorkflowApproval[]; tracks: WorkflowTrack[]; episodeTeam: WorkspaceData["episodeTeam"]; canManageEpisodes: boolean; canApproveReviews: boolean; currentPersonId: string | null }) {
  const router = useRouter();
