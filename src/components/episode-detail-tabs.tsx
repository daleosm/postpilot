...
function SimpleWorkflowPanel({ episodeId, currentStageId, currentStatus, stages, rules, approvals, exceptions, blockers, workflowSigners, workOrders, canUpdateWorkflowWork, canSubmitWorkflowTracks, canSignOffWorkflowTracks, canAuthorizeWorkflowExceptions, currentPersonId }: { episodeId?: string; currentStageId: string | null; currentStatus: string; stages: readonly WorkflowStage[]; rules: WorkflowApprovalRule[]; approvals: WorkflowApproval[]; exceptions: WorkflowException[]; blockers: WorkspaceData["workflowOperationalBlockers"]; workflowSigners: WorkspaceData["workflowSigners"]; workOrders: WorkOrder[]; canUpdateWorkflowWork: boolean; canSubmitWorkflowTracks: boolean; canSignOffWorkflowTracks: boolean; canAuthorizeWorkflowExceptions: boolean; currentPersonId: string | null }) {
  ...
  const pendingSignOffCount = Math.max(requiredRules.length - signedRuleIds.size, 0);
  ...
  return <div className="space-y-5">
    <EpisodeTabHeader eyebrow="Episode workflow" title="Workflow" description="Follow the episode through its ordered post-production stages and complete the active sign-off." />
    <section aria-label="Episode workflow" className="workflow-production-path overflow-hidden rounded-2xl border border-[#dde4df]">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#e5eae6] px-5 py-4"><div className="flex min-w-0 items-start gap-3"><span className="workflow-stage-icon workflow-stage-icon--current"><WorkflowStageIcon stage={currentStage} size={18} /></span><div><p className="text-[10px] font-semibold uppercase tracking-[.14em] text-[#728079]">Current stage</p><h3 className="mt-1 text-lg font-semibold tracking-tight text-[#314139]">{currentStage.name}</h3><p className="mt-1 text-xs text-[#75817a]">Stage {currentStage.position} of {orderedStages.length}{pendingSignOffCount ? ` · ${pendingSignOffCount} sign-off${pendingSignOffCount === 1 ? "" : "s"} pending` : ""}</p></div></div><div className="flex items-center gap-3"><div className="workflow-progress-ring" style={{ background: `conic-gradient(#4f806d ${workflowProgress}%, #e6ebe7 0)` }}><span>{workflowProgress}%</span></div><WorkflowStateBadge status={currentStatus} /></div></div>
        ...
        <div className="space-y-1">
          {orderedStages.map((stage) => {
            ...
            return <div key={stage.id} className="relative grid grid-cols-[2.25rem_minmax(0,1fr)] gap-3 sm:grid-cols-[2.75rem_minmax(0,1fr)] sm:gap-4">
              <div className="relative z-10 flex justify-center pt-3"><span className={`workflow-path-node grid h-7 w-7 place-items-center rounded-full text-[10px] font-bold ${nodeClass} ${current ? "workflow-path-node--current" : ""}`}>{completed ? <Check size={13} strokeWidth={3} /> : <WorkflowStageIcon stage={stage} size={13} />}</span></div>
              <button type="button" aria-label={`Select ${stage.name}`} aria-pressed={selected} onClick={() => { setSelectedStageId(stage.id); setMessage(""); }} className={`mb-2 flex min-w-0 items-start justify-between gap-3 rounded-xl px-3 py-2.5 text-left transition focus:outline-none focus:ring-2 focus:ring-[#87a89a] focus:ring-offset-2 ${current ? "bg-[#eef6f1]" : selected ? "bg-[#f1f6f3]" : "hover:bg-[#f5f8f5]"}`}>
                <span className="min-w-0"><span className={`block text-sm font-semibold ${current ? "text-[#315f52]" : completed ? "text-[#547866]" : "text-[#48554f]"}`}>{stage.name}</span><span className="mt-1 block text-[11px] text-[#7a8580]">Stage {stage.position} · {requiredSignOffs ? `${requiredSignOffs} required sign-off${requiredSignOffs === 1 ? "" : "s"}` : "No sign-off gate"}{activeWorkOrders.length ? ` · ${activeWorkOrders.length} active work order${activeWorkOrders.length === 1 ? "" : "s"}` : ""}</span></span>
