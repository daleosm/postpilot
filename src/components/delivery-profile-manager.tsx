...
function ProfileDialog({ profile, companies, shows }: { profile?: DeliveryProfileRecord; companies: Choice[]; shows: Choice[] }) {
  ...
  async function submit(values: ProfileValues) {
    ...
    const response = await postpilotUiFetch(profile ? `/v1/delivery-profiles/${profile.id}` : "/v1/delivery-profiles", { method: profile ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
...
function RequirementDialog({ profile, item, recipients }: { profile: DeliveryProfileRecord; item?: DeliveryProfileItemRecord; recipients: Contact[] }) {
  ...
  async function submit(values: ItemValues) {
    ...
    const url = item ? `/v1/delivery-profiles/${profile.id}/items/${item.id}` : `/v1/delivery-profiles/${profile.id}/items`;
    const response = await postpilotUiFetch(url, { method: item ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
