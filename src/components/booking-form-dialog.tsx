...
export type EditableBooking = { id: string; title: string; startsAt: Date; endsAt: Date; setupMinutes: number; handoverMinutes: number; isOption: boolean; optionRank: number | null; status: string; bookingType: string; roomId: string | null; episodeId: string | null; personId: string | null; guestPersonId?: string | null; notes: string | null };
...
function blankValues(initialStart: string): BookingValues { return { title: "", episodeId: "", roomId: "", personId: "", guestPersonId: "", startsAt: initialStart, endsAt: shiftHours(initialStart, 4), setupMinutes: 0, handoverMinutes: 0, isOption: false, status: "confirmed", bookingType: "edit", notes: "" }; }
...
function normalize(values: BookingValues) { return { ...values, status: values.isOption && values.status !== "cancelled" ? "tentative" : values.status, setupMinutes: Number(values.setupMinutes), handoverMinutes: Number(values.handoverMinutes), roomId: values.roomId || null, personId: values.personId || null, episodeId: values.episodeId || null, guestPersonId: values.guestPersonId || null, notes: values.notes || null, startsAt: new Date(values.startsAt).toISOString(), endsAt: new Date(values.endsAt).toISOString() }; }
