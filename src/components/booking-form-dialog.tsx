...
const bookingClientSchema = z.object({
  ...
  episodeId: z.string().optional(), roomId: z.string().optional(), personId: z.string().optional(), clientContactId: z.string().optional(),
...
export type BookingResources = { episodes: Array<{ id: string; label: string }>; rooms: Array<{ id: string; name: string; type: string }>; people: Array<{ id: string; name: string; role: string; availability: string; isFreelancer: boolean }>; contacts: Array<{ id: string; name: string; companyName: string; bookingClearance: string; accountStatus: string }> };
export type EditableBooking = { id: string; title: string; startsAt: Date; endsAt: Date; setupMinutes: number; handoverMinutes: number; strikeMinutes: number; status: string; bookingType: string; roomId: string | null; episodeId: string | null; personId: string | null; clientContactId?: string | null; notes: string | null };
...
export function BookingFormDialog({ resources, initialStart, booking, onClose }: { resources: BookingResources; initialStart: string; booking?: EditableBooking; onClose?: () => void }) {
  ...
  const clientContactId = useWatch({ control: form.control, name: "clientContactId" });
  const clientContact = resources.contacts.find((contact) => contact.id === clientContactId);
    ...
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#202725]/25 p-4" role="dialog" aria-modal="true" aria-label={editing ? "Edit booking" : "New booking"}>
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-[#e2e3de] bg-[#fafbf9] p-6 shadow-2xl">
        ...
        <form className="mt-6 space-y-4" onSubmit={submit}>
          ...
          <div className="grid gap-3 sm:grid-cols-2"><Field label="Episode" error={form.formState.errors.episodeId?.message}><select disabled={isAvailabilityBooking} {...form.register("episodeId")}><option value="">Not episode-linked</option>{resources.episodes.map((episode) => <option key={episode.id} value={episode.id}>{episode.label}</option>)}</select></Field><Field label="Attending client contact"><select disabled={isAvailabilityBooking} {...form.register("clientContactId")}><option value="">No client attendee</option>{resources.contacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.name} · {contact.companyName}</option>)}</select></Field></div>
          {clientContact && <div className={`rounded-lg border px-3 py-2 text-xs ${clientContact.bookingClearance === "clear" && clientContact.accountStatus === "active" ? "border-[#d9e8e0] bg-[#f0f8f3] text-[#4d7566]" : "border-[#efd6ca] bg-[#fcf1eb] text-[#984f35]"}`}><b>Booking clearance · {clearanceLabel(clientContact.bookingClearance)}</b><span className="ml-1">for {clientContact.companyName}. {clientContact.bookingClearance === "clear" && clientContact.accountStatus === "active" ? "No finance detail is shown to scheduling." : "Check with the account owner or finance before confirming this client booking."}</span></div>}
...
function blankValues(initialStart: string): BookingValues { return { title: "", episodeId: "", roomId: "", personId: "", clientContactId: "", startsAt: initialStart, endsAt: shiftHours(initialStart, 4), setupMinutes: 0, handoverMinutes: 0, strikeMinutes: 0, status: "confirmed", bookingType: "edit", notes: "" }; }
function valuesFor(booking: EditableBooking): BookingValues { return { title: booking.title, episodeId: booking.episodeId ?? "", roomId: booking.roomId ?? "", personId: booking.personId ?? "", clientContactId: booking.clientContactId ?? "", startsAt: toInput(booking.startsAt), endsAt: toInput(booking.endsAt), setupMinutes: booking.setupMinutes, handoverMinutes: booking.handoverMinutes, strikeMinutes: booking.strikeMinutes, status: booking.status as BookingValues["status"], bookingType: booking.bookingType as BookingValues["bookingType"], notes: booking.notes ?? "" }; }
function normalize(values: BookingValues) { return { ...values, setupMinutes: Number(values.setupMinutes), handoverMinutes: Number(values.handoverMinutes), strikeMinutes: Number(values.strikeMinutes), roomId: values.roomId || null, personId: values.personId || null, episodeId: values.episodeId || null, clientContactId: values.clientContactId || null, notes: values.notes || null, startsAt: new Date(values.startsAt).toISOString(), endsAt: new Date(values.endsAt).toISOString() }; }
...
function clearanceLabel(clearance: string) { return clearance === "clear" ? "Clear to book" : clearance.replaceAll("_", " "); }
