...
type GuestRole = { role: string; label: string };
const newGuestSchema = z.object({ name: z.string().trim().min(2, "Enter the guest's name.").max(120), email: z.string().email("Enter a valid work email.").max(320), personRole: z.string().min(1, "Select a post-house role.") });
type NewGuest = z.infer<typeof newGuestSchema>;
export type BookingResources = { episodes: Array<{ id: string; label: string }>; rooms: Array<{ id: string; name: string; type: string }>; people: Array<{ id: string; name: string; role: string; availability: string; isFreelancer: boolean }>; guestAccounts: GuestAccount[]; guestRoles: GuestRole[] };
export type EditableBooking = { id: string; title: string; startsAt: Date; endsAt: Date; setupMinutes: number; handoverMinutes: number; strikeMinutes: number; status: string; bookingType: string; roomId: string | null; episodeId: string | null; personId: string | null; guestPersonId?: string | null; notes: string | null };
...
export function BookingFormDialog({ resources, initialStart, booking, onClose }: { resources: BookingResources; initialStart: string; booking?: EditableBooking; onClose?: () => void }) {
  ...
  return <>
    ...
    {creatingGuest && episodeId && <NewGuestAccountDialog episodeId={episodeId} roles={resources.guestRoles} onClose={() => setCreatingGuest(false)} onCreated={(guest) => { setCreatedGuests((current) => [...current, guest]); form.setValue("guestPersonId", guest.id, { shouldDirty: true }); setGuestQuery(guest.name); setCreatingGuest(false); }} />}
  </>;
}
...
function normalize(values: BookingValues) { return { ...values, setupMinutes: Number(values.setupMinutes), handoverMinutes: Number(values.handoverMinutes), strikeMinutes: Number(values.strikeMinutes), roomId: values.roomId || null, personId: values.personId || null, episodeId: values.episodeId || null, guestPersonId: values.guestPersonId || null, notes: values.notes || null, startsAt: new Date(values.startsAt).toISOString(), endsAt: new Date(values.endsAt).toISOString() }; }
