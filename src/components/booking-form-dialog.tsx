...
function blankValues(initialStart: string): BookingValues { return { title: "", episodeId: "", budgetLineId: "", roomId: "", personId: "", guestPersonId: "", startsAt: initialStart, endsAt: shiftHours(initialStart, 4), setupMinutes: 0, handoverMinutes: 0, isOption: false, status: "confirmed", bookingType: "edit", notes: "" }; }
function valuesFor(booking: EditableBooking): BookingValues { return { title: booking.title, episodeId: booking.episodeId ?? "", budgetLineId: booking.budgetLineId ?? "", roomId: booking.roomId ?? "", personId: booking.personId ?? "", guestPersonId: booking.guestPersonId ?? "", startsAt: toInput(booking.startsAt), endsAt: toInput(booking.endsAt), setupMinutes: booking.setupMinutes, handoverMinutes: booking.handoverMinutes, isOption: booking.isOption, status: booking.status as BookingValues["status"], bookingType: booking.bookingType as BookingValues["bookingType"], notes: booking.notes ?? "" }; }
function normalize(values: BookingValues) { return { ...values, status: values.isOption && values.status !== "cancelled" ? "tentative" : values.status, setupMinutes: Number(values.setupMinutes), handoverMinutes: Number(values.handoverMinutes), roomId: values.roomId || null, personId: values.personId || null, episodeId: values.episodeId || null, budgetLineId: values.budgetLineId || null, guestPersonId: values.guestPersonId || null, notes: values.notes || null, startsAt: new Date(values.startsAt).toISOString(), endsAt: new Date(values.endsAt).toISOString() }; }
