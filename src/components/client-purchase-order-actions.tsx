...
export function ClientPurchaseOrderActions({ purchaseOrderId, status, mayApprove }: { purchaseOrderId: string; status: string; mayApprove: boolean }) {
  ...
  async function update(nextStatus: "active" | "closed" | "cancelled") { setError(null); setPending(nextStatus); try { await postpilotApiFetch(`/client-purchase-orders/${purchaseOrderId}`, { method: "PATCH", body: { status: nextStatus } }); } catch (error) { setError(error instanceof Error ? error.message : "Unable to update client PO status."); return; } finally { setPending(null); } router.refresh(); }
