...
export function CateringSettingsForm({ initialMarkup }: { initialMarkup: string | number }) {
  ...
  async function save() { setSaving(true); setMessage(""); const response = await postpilotUiFetch("/v1/settings/catering", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ markupPercent: Number(markup) }) }); const body = await response.json().catch(() => null); setSaving(false); if (!response.ok) return setMessage(body?.error ?? "Could not save catering settings."); setMessage("Saved. New receipt costs will use this markup."); router.refresh(); }
