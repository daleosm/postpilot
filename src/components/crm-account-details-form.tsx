...
type Account = { id: string; accountStatus: "active" | "on_hold" | "inactive"; bookingClearance: "clear" | "authorisation_required" | "finance_approval" | "on_hold"; accountOwnerId: string | null; nextAction: string | null; nextActionDueAt: string | null; notes: string | null };
...
export function CrmAccountDetailsForm({ account, owners }: { account: Account; owners: Array<{ id: string; name: string; role: string }> }) {
  ...
  async function save(values: Values) {
    setError("");
    const response = await postpilotUiFetch(`/v1/crm/companies/${account.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...values, accountOwnerId: values.accountOwnerId || null, nextAction: values.nextAction || null, nextActionDueAt: values.nextActionDueAt || null, notes: values.notes || null }) });
