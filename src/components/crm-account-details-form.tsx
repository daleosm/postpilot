...
export function CrmAccountDetailsForm({ account, owners }: { account: Account; owners: Array<{ id: string; name: string; role: string }> }) {
  ...
  async function save(values: Values) {
    setError("");
    const response = await fetch(`/api/crm/companies/${account.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...values,
        accountOwnerId: values.accountOwnerId || null,
...
function defaults(account: Account): Values {
  return { accountStatus: account.accountStatus, accountOwnerId: account.accountOwnerId ?? "", nextAction: account.nextAction ?? "", nextActionDueAt: account.nextActionDueAt ?? "", notes: account.notes ?? "" };
}
