...
type Resources = { rooms: Array<{ id: string; name: string; type: string }>; activeBooking: { id: string; roomId: string; roomName: string; coverage: "scheduled" | "actual_time" | "overtime_pending" } | null; activeWorkOrder: { id: string; episodeId: string; title: string } | null };
...
export function CateringRequestForm({ resources }: { resources: Resources }) {
  ...
  const activeBooking = resources.activeBooking;
  const activeWorkOrder = activeBooking ? null : resources.activeWorkOrder;
  const billingSource = activeBooking ?? activeWorkOrder;
  async function submit(event: React.FormEvent) {
    event.preventDefault();
    if (!billingSource) return setMessage("Catering requests need an active booking or assigned work order.");
    if (!item.trim() || !roomId) return setMessage("Choose a room and describe the request.");
    setSaving(true); setMessage("");
    try {
      const response = await postpilotUiFetch("/v1/catering-requests", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bookingId: activeBooking?.id ?? null, workOrderId: activeWorkOrder?.id ?? null, roomId, requestType, item, quantity, requestedFor: requestedFor ? new Date(requestedFor).toISOString() : null, notes: notes || null }) });
  ...
  return <OperationalPanel className="p-5">
    ...
    <form className="mt-5 space-y-4" onSubmit={submit}>
      <fieldset className="rounded-lg border border-[#e6e7e2] bg-[#fafbf9] p-3"><legend className="px-1 text-xs font-semibold text-[#4b5752]">Delivery location</legend><label className="block text-xs font-medium text-[#535b57]">Room<select value={roomId} onChange={(event) => setRoomId(event.target.value)} className={fieldClass} disabled={!billingSource}><option value="">Choose a room</option>{resources.rooms.map((room) => <option key={room.id} value={room.id}>{room.name} · {room.type}</option>)}</select></label><p className="mt-2 text-[11px] leading-4 text-[#858a87]">{activeBooking ? activeBooking.coverage === "overtime_pending" ? `Your booked session in ${activeBooking.roomName} has just ended. This request is covered as unreported overtime and still bills that booking’s episode.` : `Preselected from your active booking in ${activeBooking.roomName}. Change it if you moved rooms; the request still bills that booking’s episode.` : activeWorkOrder ? `Charged to your active work order: ${activeWorkOrder.title}. Choose the room where the runner should deliver it.` : "Catering requests need an active booking or assigned work order, so the cost can be charged to the correct episode."}</p></fieldset>
