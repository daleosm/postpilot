...
export function EpisodeInvoicePanel({ episodeId, readiness }: { episodeId: string; readiness: InvoiceReadiness | null }) {
  ...
  async function issueAndDownload() {
    ...
    const response = await fetch("/api/client-invoices", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ episodeId }) });
    const body = await response.json().catch(() => null);
    setIssuing(false);
    if (!response.ok) { setMessage(body?.error ?? "Could not issue this invoice."); return; }
    router.refresh();
