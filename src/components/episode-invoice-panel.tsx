...
type InvoiceReadiness = {
  ...
  bookingComponents: Array<{
    id: string; bookingId: string; bookingTitle: string; bookingDate: string | null; componentType: "room" | "person" | "fixed_fee"; commercialTreatment: CommercialTreatment;
    workOrderId: string | null; workOrderTitle: string | null;
...
export function EpisodeInvoicePanel({ episodeId, readiness, canApproveBillableCharges = true, canIssueInvoices = true }: { episodeId: string; readiness: InvoiceReadiness | null; canApproveBillableCharges?: boolean; canIssueInvoices?: boolean }) {
  ...
  return <section className="panel overflow-hidden">
    <div className="flex flex-col justify-between gap-4 border-b border-[#ebeae6] px-5 py-4 sm:flex-row sm:items-start">
      ...
      {canIssueInvoices && <Button variant="primary" onPress={issueAndDownload} isDisabled={!readiness.readyToIssue || issuing} className="bg-[#476f61] text-white disabled:opacity-50"><Send size={14} />{issuing ? "Issuing…" : "Issue & download PDF"}</Button>}
    </div>
    ...
    <BookingComponentInvoiceQueue components={readiness.bookingComponents} canApproveBillableCharges={canApproveBillableCharges} />
...
function BookingComponentInvoiceQueue({ components, canApproveBillableCharges }: { components: InvoiceReadiness["bookingComponents"]; canApproveBillableCharges: boolean }) {
  const router = useRouter();
  ...
  return <div className="border-t border-[#efeeea]">
    <div className="px-5 py-3"><p className="text-xs font-semibold text-[#59635e]">Invoice candidates</p><p className="mt-1 text-xs leading-5 text-[#858a87]">Each approved billable booking component remains separate: wet hire shows room and artist lines, dry hire shows the room only, and a project fee is one fixed-fee line. Approved overtime is shown separately.</p>{!canApproveBillableCharges && <p className="mt-2 text-xs font-medium text-[#8a6541]">You can review these charges, but a user with the billable-charge approval capability must include or exclude them.</p>}</div>
    <div className="overflow-x-auto border-t border-[#efeeea]"><div className="min-w-[850px]"><div className="grid grid-cols-[minmax(220px,1.5fr)_110px_110px_110px_140px] gap-3 bg-[#f7f7f4] px-5 py-2.5 text-[10px] font-semibold uppercase tracking-[.08em] text-[#717974]"><span>Booking / resource</span><span>Actual</span><span>Saved rate</span><span>Total</span><span>Invoice selection</span></div><div className="divide-y divide-[#efeeea]">{components.map((component) => {
      ...
      return <div key={component.id} className="grid grid-cols-[minmax(220px,1.5fr)_110px_110px_110px_140px] gap-3 px-5 py-3 text-xs text-[#59635e]"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate font-semibold text-[#39423e]">{component.resource}</p><CommercialTreatmentBadge treatment={component.commercialTreatment} /></div><p className="mt-1 truncate text-[#858a87]">{component.bookingTitle}{component.bookingDate ? ` · ${component.bookingDate}` : ""}</p>{component.workOrderTitle && <p className="mt-1 truncate text-[11px] text-[#737b77]">Work order · {component.workOrderTitle}</p>}<p className="mt-1 text-[11px] text-[#737b77]">{component.componentType === "room" ? "Room" : component.componentType === "person" ? "Artist" : "Fixed fee"} · {component.category}</p><CommercialTreatmentSummary treatment={component.commercialTreatment} className="mt-1 text-[11px] leading-4" />{component.actualOvertimeQuantity > 0 && <p className="mt-1 text-[11px] text-[#8a6541]">+ {component.actualOvertimeQuantity} {component.unit} overtime at {component.overtimeMultiplier}×</p>}</div><span>{component.actualQuantity} {component.unit}</span><span>{component.currency} {component.rate.toFixed(2)}</span><span className="font-semibold text-[#39423e]">{component.currency} {component.actualAmount.toFixed(2)}</span><div>{component.selectionStatus === "invoiced" ? <span className="font-semibold text-[#59756c]">Invoiced</span> : canApproveBillableCharges ? <div className="space-y-2">{!included && <Button size="sm" variant="primary" isDisabled={!editable || busy === component.id} onPress={() => setSelection(component, true)} className="w-full bg-[#476f61] text-white">{busy === component.id ? "Saving…" : "Include"}</Button>}{included && <span className="font-semibold text-[#59756c]">Included</span>}{(excluded || included || component.selectionStatus === "awaiting_selection") && <><input aria-label={`Exclusion reason for ${component.resource}`} value={reasonById[component.id] ?? component.selectionReason ?? ""} onChange={(event) => setReasonById((current) => ({ ...current, [component.id]: event.target.value }))} placeholder="Reason to exclude" className="h-8 w-full rounded-md border border-[#dedfda] bg-[#fcfcfa] px-2 text-[11px] text-[#45504b]" /><Button size="sm" variant="tertiary" isDisabled={!editable || busy === component.id} onPress={() => setSelection(component, false)} className="w-full">Exclude</Button></>}</div> : <span className="text-[11px] text-[#7b827e]">Review only</span>}</div></div>;
