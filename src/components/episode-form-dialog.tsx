...
const schema = z.object({ seasonId: z.string().min(1, "Select a season."), number: z.coerce.number().int().positive(), title: z.string().trim().min(1, "Enter an episode title.").max(160), productionCode: z.string().trim().max(40).optional(), airDate: z.string().optional(), deliveryDeadline: z.string().optional(), team: z.array(z.string()).default([]) });
...
export function EpisodeFormDialog({ seasons, people = [], defaultSeasonId }: { seasons: EpisodeSeason[]; people?: EpisodePerson[]; defaultSeasonId?: string }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    const load = postpilotApiFetch<{ team: Array<{ person_id: string }> }>(`/episodes/seasons/${selectedSeason}/last-episode-team`);
    load.then((data) => {
      if (!active) return;
      setLastTeam((data?.team ?? []).map((item: { personId?: string; person_id?: string }) => item.person_id ?? item.personId ?? "").filter(Boolean));
    }).catch(() => {
      if (active) setLastTeam([]);
    });
    return () => { active = false; };
  }, [selectedSeason]);
