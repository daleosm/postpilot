...
const navigation = [
  ...
  { label: "Approvals", icon: FileCheck2, href: "/review", permissions: ["manage_reviews", "update_assigned_work"] as Permission[] },
...
export async function AppSidebar() {
  ...
  const pending = context?.organization && context.person ? (await Promise.all([listWorkflowSignOffInbox(context.organization.organizationId, context.userId), listWorkOrderInbox(context.organization.organizationId, context.userId)])).reduce((total, items) => total + items.length, 0) : 0;
  const visible = permitted.filter((item): item is NonNullable<typeof item> => Boolean(item));
  const approvalItem = navigation.find((item) => item.href === "/review");
  if (pending > 0 && approvalItem && !visible.some((item) => item.href === approvalItem.href)) visible.push(approvalItem);
