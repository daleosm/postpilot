...
export function EpisodeTeamModal({ episodeId }: { episodeId: string }) {
  ...
  return <div className="mt-5"><EpisodeTeam episodeId={episodeId} assignments={data.assignments} people={data.people} canManage onChanged={() => { void load().catch((cause: Error) => setError(cause.message)); }} /></div>;
}
