...
type UserAccess = { userId: string; userName: string | null; email: string; membershipRole: "owner" | "admin" | "member" | "guest"; personId: string | null; personName: string | null; personRole: string | null; isActive: boolean | null };
...
const newUserSchema = z.object({
  ...
  membershipRole: z.enum(["admin", "member", "guest"]),
});
...
export function UserAccessManager({ users, policies }: { users: UserAccess[]; policies: Policy[] }) {
  ...
  const form = useForm<NewUser>({ resolver: zodResolver(newUserSchema), defaultValues: { name: "", email: "", personRole: policies[0]?.role ?? "", membershipRole: "member" } });
  ...
  async function create(values: NewUser) {
    ...
    form.reset({ name: "", email: "", personRole: policies[0]?.role ?? "", membershipRole: "member" });
    setOpen(false);
  ...
  return <div className="space-y-4">
    ...
    {open && <UserDialog title="Add user" onClose={() => setOpen(false)}><form className="space-y-4" onSubmit={form.handleSubmit(create)}><p className="text-sm text-[#747977]">Creates access in this post house only. The person signs in with an email one-time code.</p><Field label="Name" error={form.formState.errors.name?.message}><input {...form.register("name")} autoFocus /></Field><Field label="Work email" error={form.formState.errors.email?.message}><input type="email" {...form.register("email")} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Post-house role" error={form.formState.errors.personRole?.message}><select {...form.register("personRole")}>{policies.map((policy) => <option key={policy.role} value={policy.role}>{policy.label}</option>)}</select></Field><Field label="Account access"><select {...form.register("membershipRole")}><option value="member">Member</option><option value="admin">Administrator</option><option value="guest">Guest</option></select></Field></div><DialogActions onClose={() => setOpen(false)} saving={form.formState.isSubmitting} label="Create user" /></form></UserDialog>}

    {editing && <UserDialog title={`Edit ${editing.personName ?? editing.userName ?? editing.email}`} onClose={() => setEditing(null)}><div className="space-y-4"><p className="text-sm text-[#747977]">{editing.email}</p><div className="grid gap-4 sm:grid-cols-2"><Field label="Post-house role"><select value={editing.personRole ?? ""} onChange={(event) => setEditing({ ...editing, personRole: event.target.value })}>{policies.map((policy) => <option key={policy.role} value={policy.role}>{policy.label}</option>)}</select></Field><Field label="Account access"><select value={editing.membershipRole} onChange={(event) => setEditing({ ...editing, membershipRole: event.target.value as UserAccess["membershipRole"] })}><option value="member">Member</option><option value="admin">Administrator</option><option value="guest">Guest</option></select></Field></div><div className="flex justify-end gap-2 border-t border-[#ecebe7] pt-4"><Button variant="tertiary" onPress={() => setEditing(null)}>Cancel</Button><Button variant="primary" onPress={saveAccess} className="bg-[#263130] text-white">Save access</Button></div></div></UserDialog>}
  </div>;
}
