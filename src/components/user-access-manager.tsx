...
export function UserAccessManager({ users, policies }: { users: UserAccess[]; policies: Policy[] }) {
  ...
  const form = useForm<NewUser>({ resolver: zodResolver(newUserSchema), defaultValues: { name: "", email: "", password: "", confirmPassword: "", personRole: firstOperationalRole, membershipRole: "member" } });
  ...
  return <div className="space-y-4">
    ...
    <section className="panel overflow-hidden">
      ...
      </article>)}{!users.length && <div className="px-5 py-12 text-center"><ShieldCheck className="mx-auto text-[#a0a6a1]" size={23} /><p className="mt-3 text-sm font-medium text-[#59615d]">No user accounts yet</p><p className="mt-1 text-xs text-[#858a87]">Add the first post-house user and set their sign-in password.</p></div>}</div>
    </section>
