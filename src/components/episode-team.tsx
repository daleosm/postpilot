...
export function EpisodeTeam({ episodeId, assignments, people, canManage, onChanged }: { episodeId: string; assignments: Assignment[]; people: Person[]; canManage: boolean; onChanged?: () => void | Promise<void> }) {
  ...
  async function setSigner(assignmentId: string, isSigner: boolean) {
    setError("");
    const response = await fetch(`/api/episodes/${episodeId}/team`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ assignmentId, isSigner }) });
    ...
    return <section className="self-start rounded-xl border border-[#e5e7e3] bg-[#fafbf9] p-4">
      ...
      {assignments.length ? <div className="mt-3 grid gap-2 sm:grid-cols-2">
        {assignments.map((item) => <div key={item.id} className="flex min-w-0 items-center justify-between gap-2 rounded-lg border border-[#e8ebe7] bg-white/60 px-3 py-2.5">
          ...
          {item.isLead && <span className="shrink-0 rounded-full bg-[#e2eee6] px-1.5 py-0.5 text-[9px] font-semibold text-[#477561]">Signer</span>}
        </div>)}
  ...
  return <div className="min-w-0 rounded-lg border border-[#ecebe7] p-3">
    ...
    <p className="mt-1 text-[11px] leading-4 text-[#858a87]">Tick one person in each role to receive that role’s workflow sign-offs.</p>
    <div className="mt-2 overflow-hidden rounded border border-[#ecebe7]">
      <div className="grid grid-cols-[minmax(0,1fr)_7rem_1.75rem] items-center gap-2 border-b border-[#ecebe7] bg-[#f4f5f2] px-2 py-1.5 text-[9px] font-semibold uppercase tracking-[.06em] text-[#7d837f]">
        <span>Team member</span><span>Workflow signer</span><span />
      </div>
      {assignments.map((item) => <div key={item.id} className="grid grid-cols-[minmax(0,1fr)_7rem_1.75rem] items-center gap-2 border-b border-[#f0f0ed] bg-[#fafaf8] px-2 py-1.5 text-xs last:border-b-0">
        <span className="min-w-0 truncate font-medium text-[#46504b]">{item.name} · {item.role.replaceAll("_", " ")}</span>
        <input type="checkbox" checked={item.isLead} onChange={(event) => setSigner(item.id, event.target.checked)} aria-label={`Set ${item.name} as workflow signer`} className="justify-self-start" />
