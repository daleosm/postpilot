...
function Risk({ risk }: { risk: string }) { const tone = risk === "overdue" ? "pp-status--danger" : risk === "at_risk" ? "pp-status--warning" : "pp-status--success"; return <span className={`pp-status ${tone}`}>{risk.replaceAll("_", " ")}</span>; }
